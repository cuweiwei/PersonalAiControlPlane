...
test("Virtual Office persists an idempotent Mission and atomically commits a validated Plan", () => {
  ...
  try {
    assert.deepEqual(db.all<{ version: number }>("SELECT version FROM schema_migrations WHERE version >= 7 ORDER BY version").map((row) => row.version), [7, 8, 9, 10, 11, 12, 13, 14, 15]);
