...
test("release artifact binds the unified Control Plane image to one commit", () => {
  const root = resolve(new URL("..", import.meta.url).pathname);
  const output = mkdtempSync(join(tmpdir(), "pai-release-"));
  const commit = "a".repeat(40);
  const serverDigest = `sha256:${"b".repeat(64)}`;
  const result = spawnSync(process.execPath, ["scripts/release-artifact.mjs", "--commit", commit, "--repository", "example/pai", "--image", `ghcr.io/example/personal-ai-control-plane:sha-${commit}`, "--digest", serverDigest, "--output", output], { cwd: root, encoding: "utf8" });
  assert.equal(result.status, 0, result.stderr);
  const manifest = JSON.parse(readFileSync(join(output, `release-manifest-${commit}.json`), "utf8"));
  assert.deepEqual(manifest.images.map((image: { serviceId: string; imageDigest: string }) => [image.serviceId, image.imageDigest]), [["personal-ai-control-plane", serverDigest]]);
});
