...
- GoosePC is online and its current capability row reports `READY`, `GRANTED`, and `VERIFIED`.
- Production lifecycle smoke used the deployed Hermes MCP client to open session `01a0af2f-7ba6-7f51-a801-9be89042a1a5` directly after the capability Grant. It returned `ACTIVE`, `persistent=true`, `expiresAt=null`, and `authorizationMode=CAPABILITY_GRANT` with only `observe`/`list_windows` allowed. CP DELETE revoked it; the Worker confirmed stop and the desktop lock is `RELEASED`. No screenshot or physical input was performed.
