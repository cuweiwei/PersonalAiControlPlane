...
| 項目 | 證據 | 狀態 |
| --- | --- | --- |
| Control Plane source | `c6ec85e` — 保留缺省 `schema_version` 的 v1 intake、typed v2 task/capability guard、Codex workspace contract、Ollama bounded generation options、artifact manifest hash/bytes integrity gate、fault regressions；Compose pin `6ef0b43` | `implemented_local` |
| Control Plane CI/image | CI `35043953501` 全部通過；image `sha256:d3fd1211e8eb54d60989a6682e872ce8b6552afe6a2b8b6503a527c4aea051ec` | `ci_verified` |
| Control Plane production | Compose pin `6ef0b43`；PersonalAiControlPlane gateway validate/deploy/status 通過；running digest 與 image 一致；`/readyz`、`/healthz` 200（2026-09-16 09:27–09:28 CST） | `live_verified` |
...
| CP legacy intake live proof | production legacy `POST /api/v2/tasks` accepted `HTTP 202` as `schemaVersion=1` Task `01a0a7d4-4004-71b3-8e1f-d7f25c125dcd`，隨後以 bounded smoke cancel；保留 v1 compatibility，不代表 A15 migration 已完成 | `live_verified` |
| CP capability query live proof | snapshot `71dec626-8b36-4413-9ef3-d4e79a4652da`：`taskContractVersions=[1,2]`；現有 Worker/model inventory 如實投影 | `live_verified` |
...
| A02 | typed input、workspace、criteria、artifact hash；真 Codex Worker 修改 repo 並驗收 | CP v2 parser、workspace lock、attempt fence、artifact manifest、result/delivery 分離；`test/platform-v2-contract.test.ts`、`test/v2-core.test.ts`、`test/v2-worker.test.ts`、artifact tests；legacy intake 預設關閉且 rollback flag 有 live evidence | live text Task 已能建立／執行／持久化 validation，但不代表 Codex；`tw-timhong` workspace inventory 為空、Codex executor 未啟用，沒有真 Codex diff/test/artifact | 需 owner-approved Codex executor、明確 workspace、真實 bounded change、diff、tests、artifact、Hermes criteria acceptance 與 Portal 對照；owner：Worker + Hermes + CP |
...
| A15 | reader-first migration、legacy routing、升降版與 mapping；Goal/Skill/Schedule restart、occurrence 唯一、舊 loop 不接新版、Office 保留、restore drill | versioned CP contract、legacy parser preservation、legacy／v2 route mapping、ownership/operation catalogs、Agent Work tests、Office persistence；Hermes audit 確認 live MCP 不做 task type mapping；closed-schema/runtime payload guard 已部署 | CP deployment/Portal/Office 保留與 v1 compatibility 已 live verified；Hermes fresh Task/validation persistence 已 live；Goal/Skill/Schedule restart、legacy/new loop fence、跨 repo restore 尚未 provider verified | 需 Hermes native scheduler/Goal/Skill/Work restart、occurrence dedup、舊 CP decision loop 安全退役與明確 legacy/new ownership fence、各 repo rollback/restore drill；owner：Hermes + CP + ContextHub + Radar |
...
| Task contract、scheduler、Worker registry、attempt/run/fence | Control Plane | 版本化 parser、capability all-of、resource／workspace gate、歷史與 reconciliation | CP `c6ec85e` + digest `d3fd121...051ec` live；錯誤 tuple 已 pre-dispatch reject；缺省 schema 的 legacy intake 回 202 並投影 `schemaVersion=1`，typed v2 仍走 strict parser；artifact manifest 會驗證 declared hash、DB digest 與實體 bytes；Compose `6ef0b43` | 不把 generic 轉 shell；不以 Worker ONLINE 代替 executor/result |
