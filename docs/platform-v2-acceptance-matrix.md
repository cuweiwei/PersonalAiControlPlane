...
| 項目 | 證據 | 狀態 |
| --- | --- | --- |
| Control Plane source | `aaa6590` — typed task type/capability guard、codex workspace contract、Ollama bounded generation options、regressions；Compose pin `c0f79b4` | `implemented_local` |
| Control Plane CI/image | CI `35036600248` 全部通過；image `sha256:a4333f4c514168bb85b2fdf9a9f5ae9b06be6adcd75508df4379d383ce6ab716` | `ci_verified` |
| Control Plane production | Compose pin `c0f79b4`；PersonalAiControlPlane gateway validate/deploy/status 通過；running digest 與 image 一致；`/readyz`、`/healthz` 200 | `live_verified` |
...
| Hermes source/runtime | closed-schema/runtime validation fix 已部署；fresh operation -04 已建立 Work/Task/Run/Attempt，Task/Run succeeded 但 validation failed；ContextHub 未 configured | source `implemented_local`；provider `部分 live_verified` |
...
| A07 | 同 key 同內容重送、不同內容 conflict、舊 attempt/fence 回報 | Task service idempotency/CAS、attempt fencing、late result audit；`test/platform-v2-contract.test.ts`、`test/v2-core.test.ts` | live 舊 task `01a0a590-415e-7e0f-9f39-b0ecac5fd2b9` 保留單一 run/attempt；fresh Work 回報 `replayed=false`、CP `deduplicated=false`，projection `replayed=true` 先原樣保留；synthetic mismatch 未持久化 | 尚缺 production 同 key replay/conflict 與舊 attempt 回報案例，並需釐清 projection replay metadata 是否符合定義；owner：CP + Hermes |
...
| Task contract、scheduler、Worker registry、attempt/run/fence | Control Plane | 版本化 parser、capability all-of、resource／workspace gate、歷史與 reconciliation | CP `aaa6590` + digest `a4333f...b716` live；錯誤 tuple 已 pre-dispatch reject；Compose `c0f79b4` | 不把 generic 轉 shell；不以 Worker ONLINE 代替 executor/result |
| 實際 typed execution、workspace、process/effect evidence | Worker | 提供 offer、attempt evidence、result manifest、artifact refs | Ollama executor live inventory；`aaa6590` 明確傳 generation options；Codex executor/workspace 尚未 provider verified；fresh text result 原樣保存且 validation FAILED | 不把 Ollama inference 當 Codex workspace/diff/test |
...
1. 以新 operation key 重做正確 `llm.inference + ollama + llama3:latest` tuple；目前 transport/execution/validation persistence 已 live，但 result content 與 harmless criterion 不符，需 provider/Worker content 修正後再做新 action，不能重用 `01a0a590-415e-7e0f-9f39-b0ecac5fd2b9`。
...
在上述 evidence 尚未取得前，整合狀態應標示為「CP contract/scheduler/Portal 已部署；Hermes typed text 已完成實際 Task/Run/validation persistence 但 content criteria 失敗；跨 provider 真實閉環與 A02/A03/A05/A06/A09/A10/A12–A16 的部分驗收未完成」，不能宣稱全案完成。
