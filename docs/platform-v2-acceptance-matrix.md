...
| 項目 | 證據 | 狀態 |
| --- | --- | --- |
| Control Plane source | `ee668ed` — typed task type/capability guard、codex workspace contract、regressions | `implemented_local` |
| Control Plane CI/image | CI `35017710430` 全部通過；image `sha256:7aaf389060b2c5c406721917f3110deff960e36275814c9c60e9e64f7413cf47` | `ci_verified` |
| Control Plane production | Compose pin `fb8bbe1`；PersonalAiControlPlane gateway validate/deploy/status 通過；running digest 與 image 一致；`/readyz` 200 | `live_verified` |
...
| Hermes source/runtime | source audit 確認 `delegate_task` 原樣傳遞 task；live fresh attempt 曾被 upstream overload 阻擋，未建立 CP Task | source `implemented_local`；provider `未驗證` |
...
| Information Radar | candidate source/CI 已有，但 candidate health gate 未通過；production 保留舊 healthy digest，root-owned gateway probe 證據待 owner 提供 | `implemented_local` / `live_verified`（舊版） |
...
| A02 | typed input、workspace、criteria、artifact hash；真 Codex Worker 修改 repo 並驗收 | CP v2 parser、workspace lock、attempt fence、artifact manifest、result/delivery 分離；`test/platform-v2-contract.test.ts`、`test/v2-core.test.ts`、`test/v2-worker.test.ts`、artifact tests；本輪新增 task type guard regression | Live 只證明錯誤 tuple 被拒絕；目前 `tw-timhong` workspace inventory 為空、Codex executor 未啟用，沒有真 Codex diff/test/artifact | 需 owner-approved Codex executor、明確 workspace、真實 bounded change、diff、tests、artifact、Hermes criteria acceptance 與 Portal 對照；owner：Worker + Hermes + CP |
| A03 | GPU/runtime/model hard match；忙碌回 CAPACITY；真實批次輸入／輸出與資源觀測 | scheduler runtime/model/resource/load rules；`test/functional-ux.test.ts`、`test/v2-core.test.ts`；不存在模型與 unavailable runtime 會排除 | live capability snapshot 正確匹配 Ollama；GoosePC unverified/model unavailable 明確被排除；尚無 GPU batch | 需真 GPU Worker、批次 workload、capacity wait、CPU/RAM/I/O 與 scheduler latency 量測；owner：Worker + CP |
...
| A07 | 同 key 同內容重送、不同內容 conflict、舊 attempt/fence 回報 | Task service idempotency/CAS、attempt fencing、late result audit；`test/platform-v2-contract.test.ts`、`test/v2-core.test.ts` | live 舊 task `01a0a590-415e-7e0f-9f39-b0ecac5fd2b9` 保留單一 run/attempt；synthetic mismatch 未持久化 | 尚缺 production 同 key replay/conflict 與舊 attempt 回報案例；owner：CP + Hermes |
| A08 | 失聯、timeout、cancel ack 無 children evidence；UNKNOWN 仍占用，對帳後才重派 | UNKNOWN/effect/stop receipt、workspace lock release tests；`test/platform-v2-contract.test.ts`、`test/v2-core.test.ts`、`test/v2-worker.test.ts` | live acceptance counters `unknownExecutions=0`、`unknownAttempts=0`、`unknownSlots=0`；這不是 crash proof | 需真 Worker process crash、cancel 無 children evidence、保留 UNKNOWN、完成 stop/reconciliation 後才 release；owner：Worker + CP |
...
| A13 | artifact missing/hash mismatch/expired、假 progress；真檔可下載，異常阻止成功交付 | ArtifactStorage digest/upload/download、result manifest、progress sequence tests；`test/v2-http.test.ts`、`test/v2-worker.test.ts`、release/result tests | live 舊 failure 無 artifact，Portal 只顯示失敗；尚無真 Codex artifact | 需真 Codex diff/test artifact、hash mismatch/expiry/false progress injection 與 download verification；owner：Worker + CP + Hermes |
...
| Task contract、scheduler、Worker registry、attempt/run/fence | Control Plane | 版本化 parser、capability all-of、resource／workspace gate、歷史與 reconciliation | CP `ee668ed` + digest `7aaf...cf47` live；錯誤 tuple 已 pre-dispatch reject | 不把 generic 轉 shell；不以 Worker ONLINE 代替 executor/result |
| 實際 typed execution、workspace、process/effect evidence | Worker | 提供 offer、attempt evidence、result manifest、artifact refs | Ollama executor live inventory；Codex executor/workspace 尚未 provider verified | 不把 Ollama inference 當 Codex workspace/diff/test |
...
1. Hermes provider overload 清除後，以新 operation key 送出正確 `llm.inference + ollama + llama3:latest` tuple；驗證實際 result、criteria 與 delivery，不能重用 `01a0a590-415e-7e0f-9f39-b0ecac5fd2b9`。
...
在上述 evidence 尚未取得前，整合狀態應標示為「CP contract/scheduler/Portal 已部署；跨 provider 真實閉環與 A02/A03/A05/A06/A09/A10/A12–A16 的部分驗收未完成」，不能宣稱全案完成。
