...
| 項目 | 證據 | 狀態 |
| --- | --- | --- |
| Control Plane source | `2a9ba5a` — typed task/capability guard、legacy intake 預設退役且保留明確 rollback flag、Codex workspace contract、Ollama bounded generation options、artifact manifest hash/bytes integrity gate、fault regressions；Compose pin `09d9ff1` | `implemented_local` |
| Control Plane CI/image | CI `35043048271` 全部通過；image `sha256:0b3d1dbb7b9650b9dc805006832ba28d85510a52f8c1c1978db4f7023763559f` | `ci_verified` |
| Control Plane production | Compose pin `09d9ff1`；PersonalAiControlPlane gateway validate/deploy/status 通過；running digest 與 image 一致；`/readyz`、`/healthz` 200（2026-09-16 09:14–09:17 CST） | `live_verified` |
...
| CP legacy intake live proof | production `/api/v2/capabilities` 回 `taskContractVersions=[2]`、`legacyTaskIntake.enabled=false`；legacy `POST /api/v2/tasks` 回 `HTTP 410 LEGACY_TASK_INTAKE_DISABLED` | `live_verified` |
| CP capability query live proof | snapshot `cce6ea63-2ace-4e67-8e3e-d1cf22dae309`：`taskContractVersions=[2]`、`legacyTaskIntake.enabled=false`；現有 Worker/model inventory 如實投影 | `live_verified` |
...
| A15 | reader-first migration、legacy routing、升降版與 mapping；Goal/Skill/Schedule restart、occurrence 唯一、舊 loop 不接新版、Office 保留、restore drill | versioned CP contract、legacy parser preservation、legacy intake default-off rollback flag、ownership/operation catalogs、Agent Work tests、Office persistence；Hermes audit 確認 live MCP 不做 task type mapping；closed-schema/runtime payload guard 已部署 | CP deployment/Portal/Office 保留與 legacy intake default-off 已 live verified；Hermes fresh Task/validation persistence 已 live；Goal/Skill/Schedule restart、跨 repo restore 尚未 provider verified | 需 Hermes native scheduler/Goal/Skill/Work restart、occurrence dedup、legacy/new loop fence、各 repo rollback/restore drill；owner：Hermes + CP + ContextHub + Radar |
...
| Task contract、scheduler、Worker registry、attempt/run/fence | Control Plane | 版本化 parser、capability all-of、resource／workspace gate、歷史與 reconciliation | CP `2a9ba5a` + digest `0b3d1db...3559f` live；錯誤 tuple 已 pre-dispatch reject；legacy intake 預設回 410 且 capability 宣告 `[2]`；artifact manifest 會驗證 declared hash、DB digest 與實體 bytes；Compose `09d9ff1` | 不把 generic 轉 shell；不以 Worker ONLINE 代替 executor/result |
