...
- 多人即時遊戲、物理碰撞、語音會議或自由漫遊。2026-09-08 已核准的固定路徑 3D 狀態動畫納入 Office 顯示範圍。
...
2026-09-08：owner 已確認寫實辦公室草稿與人物動畫方向，Office UI 改採 React 搭配延遲載入的 Three.js / WebGL 場景。木質家具、玻璃主管區、工作席、會議桌、看板與成果櫃都是獨立 3D 物件；固定斜俯視鏡頭支援有限旋轉、縮放與聚焦。成員與設備可點選，也有等效的鍵盤操作與簡潔檢視。此變更擴充 3D 呈現，不增加自由漫遊、碰撞引擎或虛構 agent 執行。實作及狀態證據規則見 [沉浸式 Office UI](virtual-office-immersive-ui.md)。

下圖保留為功能區域的邏輯示意，實際視覺以 3D 場景為準。
