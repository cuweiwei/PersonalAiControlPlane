...
| Worker and capability mesh | Signed frames, sequence/replay/clock checks, device-initiated hashed-challenge enrollment finalization, short-lived credential rotation, authenticated HTTP poll/events channel, durable connection/message queue, capability discovery/grant separation, filesystem and shell profiles including absolute-path privilege-command rejection, exact job offers, drain/revoke state, outbound polling, durable accept-before-execute, local concurrency, checkpoint/result persistence, replay-safe result resend, Worker daemon/CLI and Codex exec adapter skeleton | `test/worker.test.ts`, `test/worker-runtime.test.ts`, `test/worker-channel.test.ts`, `apps/worker/src/*` |
...
| Owner API and Portal | Owner-safe goal/plan/DAG/attempt/checkpoint/reconciliation, approval, schedule, worker/capability/enrollment, compute/quota, conversation/job, connector, credential, policy, audit and system projections; single `/home` entry; `/systems` for Personal AI and independent external links; read-only ContextHub `/memory` projection; Hermes status plus independent-site link; durable local Orchestrator mutations; SSE cursor snapshots; keyboard/mobile/reduced-motion support | `test/http.test.ts`, `test/control-web.test.ts`, `test/private-edge.test.ts`, `npm run build:web` |
...
The single-container topology is currently `implemented_local`: source, tests, Docker build definition, Compose shape, and single-image CI contract have been updated. The historical live evidence below still describes the previous three-container release until a new immutable image is published and promoted through the NAS deployment gateway.
...
The Worker channel vertical slice now supports device-generated enrollment requests, owner approval, proof-of-possession finalization, 24-hour credential rotation, signed HTTP poll/events, WSS-primary transport with HTTP fallback, durable connection generations and inbound replay protection. `start` is bootstrap-aware: it creates the enrollment request, waits for approval, finalizes proof automatically, and then starts the user-session daemon against a registered Git repository; `enroll`/`enroll-finalize` remain explicit recovery commands. The current development CLI deliberately reports `file-fallback` for key/credential storage and uses the Codex CLI adapter; neither is production evidence for native OS-vault storage, signed desktop packages, production edge WSS/TLS acceptance, or provider acceptance.

The self-edge integration is `implemented_local` only until a new immutable image is promoted. Personal AI strips client-supplied `x-pai-*`, cookie, and CSRF headers at the edge, runs its Identity forward-auth subrequest for protected routes, and injects only verified non-secret identity headers. Worker HTTP/WebSocket routes use signed worker credentials and never browser sessions. ContextHub maps the forwarded owner UUID only when an enrolled `personal-ai` web principal is linked to a human client, and only safe methods reach the Memory projection. `/infrastructure` and its Portal API return `404`; no Portal request reaches AI Home Platform.
