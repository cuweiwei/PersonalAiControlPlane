...
The Worker channel vertical slice now supports device-generated enrollment requests, owner approval, proof-of-possession finalization, 24-hour credential rotation, signed HTTP poll/events, durable connection generations and inbound replay protection. `npm run worker:cli -- enroll` and `enroll-finalize` exercise the local flow; `start` runs the user-session daemon against a registered Git repository. The current development CLI deliberately reports `file-fallback` for key/credential storage and uses the Codex CLI adapter; neither is production evidence for native OS-vault storage, signed desktop packages, WSS, or provider acceptance.
...
| `DD-02` / `DD-03` machine control | Native Keychain/Credential Manager signer, signed desktop installers, WSS primary transport, physical-device capability/provider evidence, per-device CUA isolation, and safe wake/sleep proof | Device enrollment/HTTP poll is locally implemented, but production readiness remains false while the native vault, signed package, WSS and physical acceptance evidence are absent |
