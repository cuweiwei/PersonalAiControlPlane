...
const statusLabels: Record<string, string> = { QUEUED: "等待執行", ASSIGNED: "已派送", RUNNING: "執行中", STARTED: "執行中", ADMITTED: "已接案", NOT_STARTED: "尚未執行", APPLIED: "已套用", SUCCEEDED: "已完成", FAILED: "失敗", CANCELLED: "已取消", ONLINE: "線上", OFFLINE: "離線", DISABLED: "已停用", READY: "可用", HEALTHY: "健康", UNAVAILABLE: "目前無法使用", DEGRADED: "降級", UNKNOWN: "未知", OBSERVED: "已觀測", TERMINAL_CONFIRMED: "終態已確認", STOP_RECONCILIATION: "等待停機對帳", RECONCILIATION_REQUIRED: "需要對帳", NONE_CONFIRMED: "無副作用已確認", RELEASED: "已釋放", RELEASING: "釋放中", RESERVED: "已預留", HELD: "資源占用中", LOADED: "已載入", AVAILABLE: "可取得", PARTIAL: "部分可取得", PENDING: "等待中", RETRY_WAIT: "等待重試", IN_FLIGHT: "投遞中", DELIVERED: "已交付", ATTENTION: "需要處理" };
...
function Layout({ children, path, refresh }: { children: React.ReactNode; path: string; refresh(): void }) {
  ...
  return h(
    ...
    h(
      ...
      h("div", { className: "portal-connection", role: "status" }, h("i", { "aria-hidden": true }), h("span", null, "工作空間"), h("small", null, "v2")),
    ),
...
function PageHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: React.ReactNode }) {
  return h("div", { className: "section-heading" }, h("div", null, h("p", { className: "eyebrow" }, eyebrow), h("h1", null, title), description ? h("p", null, description) : null), action ?? null);
}
...
function Home({ refreshVersion }: { refreshVersion: number }) {
  ...
  const visibleTasks = [...recent, ...latest].filter((task: Item, index, all) => all.findIndex((other: Item) => other.id === task.id) === index).slice(0, 5);
  return h(React.Fragment, null,
    h("section", { className: "portal-hero" },
      h("div", { className: "hero-copy" }, h("span", { className: "hero-monogram", "aria-hidden": true }, "P"), h("div", null,
        h("p", { className: "eyebrow" }, "WORKSPACE OVERVIEW / V2"),
        h("h1", null, "今天，一起完成。"),
        h("p", null, "Hermes 協調團隊；Control Plane 保留每一筆執行、成果與交付證據。"))),
      h("div", { className: "hero-actions" }, h("a", { className: "button-link", href: "/missions/new" }, "＋ 交辦工作"), h("a", { className: "button-link secondary", href: "/office" }, "開啟 Virtual Office →"))),
    h("section", { className: "metric-grid" },
      h(Card, { title: "Hermes", value: systems.find((item: Item) => item.id === "hermes")?.status ?? "UNKNOWN", detail: "目前觀測" }),
      h(Card, { title: "ContextHub", value: systems.find((item: Item) => item.id === "contexthub")?.status ?? "UNKNOWN", detail: "目前觀測" }),
      h(Card, { title: "正在執行", value: data.running?.items ?? 0, detail: "所有期間" }),
      h(Card, { title: "今日已完成", value: summary.countsByStatus?.SUCCEEDED ?? 0, detail: "最近 24 小時" })),
    h("section", { className: "workspace-grid" },
      ...
      h("aside", { className: "card attention-panel" },
        h("p", { className: "eyebrow" }, "NEEDS YOUR ATTENTION"), h("h2", null, attention.length ? "有工作需要查看" : "目前沒有需要處理的工作"), h("p", null, attention.length ? "補齊資訊或處理阻礙，團隊就能接著往下做。" : "Control Plane 目前沒有回報需要你介入的事項。"),
        attention.slice(0, 5).map((item: Item) => h("div", { className: "attention-row", key: item.taskId }, h(Status, { value: "ATTENTION" }), h("div", { className: "grow" }, h("a", { href: `/tasks/${item.taskId}` }, item.taskId), h("small", null, item.primaryReason))),
        h("a", { className: "button-link secondary full-action", href: "/attention" }, "查看待處理 →"),
        h("div", { className: "subsection-heading result-heading" }, h("h3", null, "最近成果")),
        latest.slice(0, 2).map((item: Item) => h("div", { className: "result-row", key: item.id }, h("strong", null, item.title), h("small", null, `${statusLabels[item.status] ?? item.status} · ${time(item.createdAt)}`)))),
    ),
    ),
    h("section", { className: "activity-strip" }, h("span", { className: "activity-title" }, "最近動態"), visibleTasks.slice(0, 3).map((task: Item) => h("span", { key: `activity-${task.id}` }, `${time(task.createdAt)} · ${task.title}`))),
  );
}
...
function Tasks({ refreshVersion }: { refreshVersion: number }) {
  ...
  const [filter, setFilter] = useState(initialQuery.get("status") ?? ""); const [search, setSearch] = useState(initialQuery.get("search") ?? ""); const [cursor, setCursor] = useState<string | null>(initialQuery.get("cursor")); const [page, setPage] = useState<Item | null>(null); const [selectedId, setSelectedId] = useState<string | null>(null);
  useEffect(() => { const params = new URLSearchParams(); params.set("purpose", "USER"); params.set("limit", "50"); if (filter) params.set("status", filter); if (search) params.set("search", search); if (cursor) params.set("cursor", cursor); window.history.replaceState({}, "", `/tasks?${params}`); setItems(null); request(`/api/v2/tasks?${params}`).then((value) => { const nextItems = value.items ?? []; setItems(nextItems); setSelectedId((current) => current && nextItems.some((item: Item) => item.id === current) ? current : nextItems[0]?.id ?? null); setPage(value.page ?? null); }).catch(setError); }, [filter, search, cursor, refreshVersion]);
  if (!items) return error ? h(ErrorPanel, { error }) : h(Loading);
  const selected = items.find((item) => item.id === selectedId) ?? items[0];
  const inspector = selected ? h("aside", { className: "inspector card" }, h("p", { className: "eyebrow" }, "TASK INSPECTOR"), h("h2", null, selected.title), h(Status, { value: selected.status }), h("div", { className: "inspector-facts" }, h("div", null, h("span", null, "執行裝置"), h("strong", null, display(selected.execution?.workerId ?? "自動選擇"))), h("div", null, h("span", null, "任務類型"), h("strong", null, display(selected.taskType))), h("div", null, h("span", null, "優先級"), h("strong", null, display(selected.priority))), h("div", null, h("span", null, "建立時間"), h("strong", null, time(selected.createdAt)))), h("div", { className: "inspector-divider" }), h("h3", null, "下一步"), h("p", null, selected.status === "FAILED" ? "檢視失敗原因並決定是否重新執行。" : selected.status === "SUCCEEDED" ? "查看成果與外部交付回執。" : "等待執行回報，必要時可進入詳情頁控制。"), h("a", { className: "button-link secondary full-action", href: `/tasks/${selected.id}` }, "開啟完整任務 →")) : h("aside", { className: "inspector card" }, h("p", { className: "empty-copy" }, "選取任務後查看執行詳情。"));
  return h(React.Fragment, null,
    ...
    h("div", { className: "task-layout" }, h("div", { className: "table-wrap" }, h("table", null,
      h("thead", null, h("tr", null, ["任務", "狀態", "類型", "實際 Worker / 模型", "優先級", "建立時間"].map((header) => h("th", { key: header }, header)))),
      h("tbody", null, items.map((item) => h("tr", { key: item.id, className: item.id === selected?.id ? "selected" : undefined, onClick: () => setSelectedId(item.id) }, h("td", null, h("a", { href: `/tasks/${item.id}` }, item.title), h("small", null, item.id)), h("td", null, h(Status, { value: item.status })), h("td", null, item.taskType), h("td", null, `${display(item.execution?.workerId ?? "auto")} / ${display(item.execution?.model?.name ?? "any")}`), h("td", null, item.priority), h("td", null, time(item.createdAt))))))), inspector),
...
function Workers({ refreshVersion }: { refreshVersion: number }) {
  ...
  return h(React.Fragment, null,
    h(PageHeader, { eyebrow: "WORKER DIRECTORY", title: "執行裝置", description: `${allWorkers.length} 台已註冊 · ${allWorkers.filter((item) => item.connection?.state === "ONLINE").length} 台線上`, action: h("a", { className: "button-link", href: "/workers/new" }, "＋ 新增執行裝置") }),
...
function Computers({ refreshVersion }: { refreshVersion: number }) {
  const [data, setData] = useState<Item | null>(null); const [sessions, setSessions] = useState<Item[]>([]); const [error, setError] = useState<unknown>(null); const [selected, setSelected] = useState(""); const [session, setSession] = useState<Item | null>(null); const [busy, setBusy] = useState(false);
  const requestedSessionId = typeof window === "undefined" ? "" : new URLSearchParams(window.location.search).get("session_id") ?? "";
  const reload = useCallback(() => { Promise.all([request("/api/v2/computers"), request("/api/v2/computer-sessions")]).then(([value, sessionValue]) => { setData(value); setSessions(sessionValue.items ?? []); if (!selected && value.items?.[0]?.workerId) setSelected(value.items[0].workerId); }).catch(setError); }, [selected]);
  ...
  const open = async (workerId = selected) => { if (!workerId) return; setBusy(true); try { const result = await request("/api/v2/computer-sessions", { method: "POST", headers: { "idempotency-key": `computer-session-${workerId}-${Date.now()}` }, body: JSON.stringify({ worker_id: workerId, desktop_kind: "desktop", capture_scope: "desktop", allowed_operations: ["observe", "list_windows", "click", "move", "drag", "scroll", "type_text", "press_key", "hotkey", "launch_app", "focus_window"], allowed_apps: ["*"], delivery_modes: ["background"], model: { provider: "hermes", id: "configured" } }) }); setSession(await request(`/api/v2/computer-sessions/${result.id}`)); await reload(); } catch (reason) { setError(reason); } finally { setBusy(false); } };
  ...
  const deleteSession = async (item: Item) => { setBusy(true); try { const value = await request(`/api/v2/computer-sessions/${encodeURIComponent(item.id)}`, { method: "DELETE", headers: { "idempotency-key": `computer-delete-${item.id}-${Date.now()}` }, body: JSON.stringify({ expected_revision: item.revision }) }); if (session?.id === item.id) setSession(value); await reload(); } catch (reason) { setError(reason); } finally { setBusy(false); } };
  if (!data) return error ? h(ErrorPanel, { error }) : h(Loading);
  const items = data.items ?? [];
  return h(React.Fragment, null,
    h(PageHeader, { eyebrow: "COMPUTER USE", title: "Computer Use", description: "已 Grant computer.use 的 Hermes 可直接建立或重用桌面 session。Session 不會因閒置或時間自動過期；你可在此撤銷並刪除。" }),
    error ? h(ErrorPanel, { error }) : null,
    h("section", { className: "card-grid" }, items.map((item: Item) => h("article", { className: "card", key: item.workerId }, h("div", { className: "card-title-row" }, h("h2", null, item.name), h(Status, { value: item.online ? "ONLINE" : item.status })), h("p", null, `${item.platform} · ${item.capabilities?.map((capability: Item) => `${capability.capability} ${capability.grantStatus}`).join(", ") || "尚未授權 computer.use"}`), h("p", null, `桌面：${(item.desktops ?? []).map((desktop: Item) => `${desktop.desktopId} (${desktop.state})`).join(", ") || "—"}`), h("button", { type: "button", disabled: busy || !item.online || !item.capabilities?.some((capability: Item) => capability.capability === "computer.use" && capability.grantStatus === "GRANTED" && capability.evidenceState === "VERIFIED"), onClick: () => { setSelected(item.workerId); void open(item.workerId); } }, "建立桌面 session")))),
    h("section", { className: "card" },
      h("h2", null, "Sessions"),
      sessions.length ? sessions.map((item: Item) => h("article", { className: "registration", key: item.id },
        h("div", { className: "card-title-row" }, h("strong", null, `${item.workerId} · ${item.desktopId}`), h(Status, { value: item.state })),
        h("p", null, `${item.id} · ${item.persistent ? "無期限" : `expires ${time(item.expiresAt)}`} · ${item.actionCount ?? 0} actions · stop ${item.stopState ?? "UNKNOWN"}`),
        h("div", { className: "actions" },
          h("button", { type: "button", disabled: busy, onClick: () => { setSession(item); window.history.replaceState(null, "", `/computers?session_id=${encodeURIComponent(item.id)}`); } }, "檢視"),
          h("button", { className: "danger", type: "button", disabled: busy, onClick: () => window.confirm("撤銷並刪除此 computer session？操作與稽核證據會保留。") && void deleteSession(item) }, "刪除 session"),
        ),
      )) : h("p", null, "目前沒有 computer sessions."),
    ),
    session ? h("section", { className: "card" }, h("h2", null, `Session ${session.id}`), h(Status, { value: session.state }), h(Details, { item: { workerId: session.workerId, desktopId: session.desktopId, desktopEpoch: session.desktopEpoch, scope: session.scope, model: session.model, authorizationMode: session.authorizationMode, persistent: session.persistent, expiresAt: time(session.expiresAt), nextSequence: session.nextSequence, revision: session.revision, lock: session.lock } }), ["ACTIVE", "PAUSED"].includes(session.state) ? h("div", { className: "actions" }, h("button", { type: "button", disabled: busy, onClick: () => void act(`/api/v2/computer-sessions/${session.id}/control`, { action: session.state === "PAUSED" ? "resume" : "pause", expected_revision: session.revision }) }, session.state === "PAUSED" ? "Resume" : "Pause"), h("button", { type: "button", disabled: busy, onClick: () => void act(`/api/v2/computer-sessions/${session.id}/control`, { action: "close", expected_revision: session.revision }) }, "Close"), h("button", { className: "danger", type: "button", disabled: busy, onClick: () => void deleteSession(session) }, "刪除 session")) : null) : null,
  );
}
...
function WorkerDetail({ id, refreshVersion }: { id: string; refreshVersion: number }) {
  ...
  return h(React.Fragment, null,
    h(PageHeader, { eyebrow: "WORKER DETAIL", title: item.name, description: `${item.platform} · ${item.hostname ?? "—"} · ${item.id}`, action: h(Status, { value: item.connection?.state ?? item.status }) }),
...
function AgentWorkPage({ kind, refreshVersion }: { kind: "skills" | "goals" | "routines" | "attention" | "browser-sessions"; refreshVersion: number }) {
  ...
  return h(React.Fragment, null,
    h(PageHeader, { eyebrow: "PERSONAL AGENT WORK", title: labels[kind], description: "保留 Control Plane 已持久化的工作資產與目前能力證據。", action: h(Status, { value: capability?.available ? "READY" : capability?.configured ? "UNAVAILABLE" : "DISABLED" }) }),
...
function OfficeMembers({ refreshVersion }: { refreshVersion: number }) {
  ...
  return h(React.Fragment, null, h(PageHeader, { eyebrow: "OFFICE MEMBERS", title: "成員與綁定", description: "角色與 binding 是 Mission admission 的來源；變更前請確認 Worker capability、model 與 workspace scope。", action: h("a", { className: "button-link secondary", href: "/office" }, "回到 Office") }), h("div", { className: "card-grid" }, cards));
}
...
function MissionList({ refreshVersion }: { refreshVersion: number }) {
  ...
  const bucketFor = (item: Item) => {
    const current = item.run?.phase;
    if (current === "COMPLETED") return "completed";
    if (item.run?.waitSummary?.reason || ["FAILED", "CANCELLED"].includes(current)) return "attention";
    if (["EXECUTING", "REVIEWING"].includes(current)) return "active";
    return "todo";
  };
  const buckets = [{ key: "todo", label: "待辦" }, { key: "active", label: "進行中" }, { key: "attention", label: "等你處理" }, { key: "completed", label: "已完成" }];
  const board = h("div", { className: "mission-board" }, buckets.map(({ key, label }) => {
    const bucketItems = items.filter((item) => bucketFor(item) === key);
    return h("section", { className: "mission-column", key }, h("div", { className: "mission-column-heading" }, h("h2", null, label), h("span", null, bucketItems.length)), bucketItems.length ? bucketItems.map((item) => h("article", { className: "mission-card", key: item.id }, h(Status, { value: item.run?.phase ?? "UNKNOWN" }), h("h3", null, h("a", { href: `/missions/${item.id}` }, item.title)), h("p", null, item.goal), item.run?.waitSummary?.reason ? h("small", null, `等待：${item.run.waitSummary.reason}`) : null, h("time", null, time(item.updatedAt)))) : h("p", { className: "empty-copy" }, "目前沒有交辦"));
  }));
  const phases = ["PLANNING", "EXECUTING", "REVIEWING", "COMPLETED", "FAILED", "CANCELLED"].map((value) => h("option", { key: value, value }, value));
  return h(React.Fragment, null, h("div", { className: "section-heading" }, h("div", null, h("p", { className: "eyebrow" }, "MISSION CONTROL"), h("h1", null, "團隊交辦"), h("p", null, "每一份交辦，都有清楚的進度與下一步。")), h("div", { className: "actions" }, h("select", { value: phase, onChange: (event: React.ChangeEvent<HTMLSelectElement>) => setPhase(event.target.value) }, h("option", { value: "" }, "全部階段"), phases), h("a", { className: "button-link", href: "/missions/new" }, "＋ 建立 Mission"))), h("div", { className: "mission-summary" }, h(Card, { title: "全部交辦", value: items.length }), h(Card, { title: "進行中", value: items.filter((item) => bucketFor(item) === "active").length }), h(Card, { title: "等你處理", value: items.filter((item) => bucketFor(item) === "attention").length }), h(Card, { title: "已完成", value: items.filter((item) => bucketFor(item) === "completed").length })), items.length === 0 ? h("p", { className: "notice" }, "目前沒有符合條件的 Mission。") : board);
}
