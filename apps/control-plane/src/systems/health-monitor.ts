...
export class HealthMonitor {
  ...
  list(selfReady: boolean): Record<string, unknown>[] { const external = this.db.all<Row>("SELECT s.*, h.status, h.latency_ms, h.message, h.checked_at FROM systems s LEFT JOIN system_health h ON h.system_id = s.id ORDER BY s.id").map((row) => ({ id: row.id, name: row.name, type: row.type, baseUrl: row.base_url, healthPath: row.health_path, entryUrl: row.entry_url ?? null, status: row.status ?? "UNKNOWN", latencyMs: row.latency_ms ?? null, message: row.message ?? null, checkedAt: iso(row.checked_at) })); return [{ id: "control-plane", name: "PersonalAiControlPlane", type: "control-plane", entryUrl: process.env.PAI_CONTROL_PLANE_ENTRY_URL ?? null, status: selfReady ? "HEALTHY" : "DEGRADED", latencyMs: 0, checkedAt: new Date().toISOString() }, ...external]; }
}
