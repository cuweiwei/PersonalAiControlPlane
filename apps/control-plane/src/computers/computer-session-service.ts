...
const NO_SESSION_EXPIRY = Number.MAX_SAFE_INTEGER;
...
function publicSession(row: Row): Record<string, unknown> {
  ...
  const persistent = Number(row.persistent ?? 0) === 1;
  return { id: row.id, principal: row.principal, workRef: parse(row.work_ref_json, null), workerId: row.worker_id, desktopId: row.desktop_id, desktopEpoch: Number(row.desktop_epoch), descriptorHash: row.descriptor_hash, scope, model, state: row.state, revision: Number(row.revision), persistent, authorizationMode: row.authorization_mode ?? "SESSION_APPROVAL", expiresAt: persistent ? null : iso(row.expires_at), idleExpiresAt: persistent ? null : iso(row.idle_expires_at), maxActions: Number(row.max_actions) > 0 ? Number(row.max_actions) : null, actionCount: Number(row.action_count), stopState: row.stop_state, createdAt: iso(row.created_at), approvedAt: iso(row.approved_at), closedAt: iso(row.closed_at), deletedAt: iso(row.deleted_at), updatedAt: iso(row.updated_at) };
}
...
export class ComputerSessionService {
  ...
  listComputers(now = Date.now()): Record<string, unknown>[] {
    ...
    return workers.map((worker) => {
      ...
      const active = this.db.one<Row>("SELECT id, state, expires_at, persistent, revision FROM computer_sessions WHERE worker_id = ? AND state IN ('OPENING','ACTIVE','PAUSED','CLOSING') ORDER BY created_at DESC LIMIT 1", worker.id);
      return { workerId: worker.id, name: worker.name, platform: worker.platform, hostname: worker.hostname, status: worker.status, online: worker.status === "ONLINE" && Number(worker.enabled) === 1 && Number(worker.drain) === 0, capabilities, desktops: advertisedDesktops, activeSession: active ? { id: active.id, state: active.state, revision: Number(active.revision), expiresAt: Number(active.persistent ?? 0) === 1 ? null : iso(active.expires_at) } : null, observedAt: iso(now) };
    });
  }
  ...
  get(id: string, now = Date.now()): Record<string, unknown> | undefined {
    ...
    if (Number(row.persistent ?? 0) !== 1 && ["PENDING_APPROVAL", "OPENING", "ACTIVE", "PAUSED"].includes(String(row.state)) && (Number(row.expires_at) <= now || Number(row.idle_expires_at) <= now)) this.expire(id, now);
    ...
    result.nextSequence = Number(this.db.one<Row>("SELECT COALESCE(MAX(sequence), 0) AS value FROM computer_operations WHERE session_id = ?", id)?.value ?? 0) + 1;
    return result;
  }

  listSessions(principal = "owner", now = Date.now()): Record<string, unknown>[] {
    const rows = this.db.all<Row>("SELECT id FROM computer_sessions WHERE principal = ? AND (deleted_at IS NULL OR (state = 'REVOKED' AND stop_state = 'UNKNOWN')) ORDER BY updated_at DESC, id DESC LIMIT 100", principal);
    return rows.map((row) => this.get(String(row.id), now)).filter((session): session is Record<string, unknown> => Boolean(session));
  }
  ...
  create(input: ComputerSessionCreate, idempotencyKey: string, now = Date.now()): Record<string, unknown> {
    ...
    if (String(worker.status) !== "ONLINE" || Number(worker.enabled) !== 1 || Number(worker.drain) !== 0) throw new Error("COMPUTER_WORKER_UNAVAILABLE");
    ...
    if (!Number.isInteger(maxActions) || maxActions < 0 || maxActions > 100) throw new Error("INVALID_SESSION_ACTION_LIMIT");
    ...
    const lock = this.db.one<Row>("SELECT * FROM computer_resource_locks WHERE worker_id = ? AND desktop_id = ?", input.workerId, desktopId);
    if (lock && lock.state !== "RELEASED") {
      const existing = this.db.one<Row>("SELECT * FROM computer_sessions WHERE id = ?", lock.session_id);
      if (lock.state === "HELD" && existing?.state === "ACTIVE" && String(existing.principal) === (text(input.principal, 120) || "owner") && Number(existing.persistent ?? 0) === 1 && String(existing.scope_json) === JSON.stringify(scopeValue) && String(existing.model_json) === JSON.stringify(model)) {
        const result = { ...this.get(String(existing.id), now)!, reused: true };
    ...
    const id = uuidv7(now); const expiresAt = NO_SESSION_EXPIRY; const result = { id, state: "ACTIVE", workerId: input.workerId, desktopId, desktopEpoch: Number(desktop.desktop_epoch), scope: scopeValue, scopeHash: scopeValue.scope_hash, model, revision: 1, persistent: true, authorizationMode: "CAPABILITY_GRANT", expiresAt: null, idleExpiresAt: null, maxActions: maxActions || null, actionCount: 0, reused: false, nextSequence: 1 };
    this.db.transaction(() => {
      this.db.run("INSERT INTO computer_sessions(id, principal, work_ref_json, worker_id, desktop_id, desktop_epoch, descriptor_hash, scope_json, model_json, state, persistent, authorization_mode, revision, expires_at, idle_expires_at, max_actions, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'ACTIVE', 1, 'CAPABILITY_GRANT', 1, ?, ?, ?, ?, ?)", id, text(input.principal, 120) || "owner", input.workRef ? JSON.stringify(input.workRef) : null, input.workerId, desktopId, Number(desktop.desktop_epoch), descriptorHash, JSON.stringify(scopeValue), JSON.stringify(model), expiresAt, expiresAt, maxActions, now, now);
      if (lock) this.db.run("UPDATE computer_resource_locks SET session_id = ?, desktop_epoch = ?, state = 'HELD', stop_state = 'UNKNOWN', acquired_at = ?, released_at = NULL, updated_at = ? WHERE worker_id = ? AND desktop_id = ? AND state = 'RELEASED'", id, Number(desktop.desktop_epoch), now, now, input.workerId, desktopId);
      else this.db.run("INSERT INTO computer_resource_locks(worker_id, desktop_id, session_id, desktop_epoch, state, stop_state, acquired_at, updated_at) VALUES (?, ?, ?, ?, 'HELD', 'UNKNOWN', ?, ?)", input.workerId, desktopId, id, Number(desktop.desktop_epoch), now, now);
      this.db.run("INSERT INTO operation_receipts(scope, operation_key, request_hash, status_code, response_json, created_at, retain_until) VALUES (?, ?, ?, 201, ?, ?, ?)", scope, idempotencyKey, requestHash, JSON.stringify(result), now, now + 86_400_000);
    });
    this.controller?.computerLease?.(input.workerId, id, 1, Number(desktop.desktop_epoch));
    this.audit(text(input.principal, 120) || "owner", "computer.session.created", id, { scopeHash: scopeValue.scope_hash, authorizationMode: "CAPABILITY_GRANT" }, now);
    return result;
  }
  ...
  deleteSession(id: string, expectedRevision: number, actor = "owner", now = Date.now()): Record<string, unknown> {
    const row = this.db.one<Row>("SELECT * FROM computer_sessions WHERE id = ?", id); if (!row) throw new Error("COMPUTER_SESSION_NOT_FOUND"); if (Number(row.revision) !== expectedRevision) throw new Error("SESSION_CHANGED");
    this.assertPrincipal(row, actor);
    if (row.deleted_at) return this.get(id, now)!;
    if (["PENDING_APPROVAL", "OPENING", "ACTIVE", "PAUSED", "CLOSING"].includes(String(row.state))) this.control(id, "revoke", expectedRevision, actor, now);
    this.db.run("UPDATE computer_sessions SET deleted_at = ?, updated_at = ? WHERE id = ? AND revision = ?", now, now, id, expectedRevision + (["PENDING_APPROVAL", "OPENING", "ACTIVE", "PAUSED", "CLOSING"].includes(String(row.state)) ? 1 : 0));
    this.audit(actor, "computer.session.deleted", id, { state: row.state }, now);
    return this.get(id, now)!;
  }

  renewLeases(now = Date.now()): number {
    const stale = this.db.all<Row>("SELECT id FROM computer_sessions WHERE persistent = 0 AND state IN ('ACTIVE','PAUSED') AND (expires_at <= ? OR idle_expires_at <= ?)", now, now);
    for (const row of stale) this.expire(String(row.id), now);
    const rows = this.db.all<Row>("SELECT s.id, s.worker_id, s.revision, s.desktop_epoch, l.desktop_id FROM computer_sessions s JOIN computer_resource_locks l ON l.session_id = s.id WHERE s.state IN ('ACTIVE','PAUSED') AND l.state = 'HELD' AND (s.persistent = 1 OR s.expires_at > ?)", now);
  ...
  prepareTask(input: CreateTaskInput, actor = "owner", now = Date.now()): CreateTaskInput {
    ...
    const row = this.db.one<Row>("SELECT * FROM computer_sessions WHERE id = ?", sessionId); if (!row) throw new Error("COMPUTER_SESSION_NOT_FOUND"); if (row.state !== "ACTIVE") throw new Error("SESSION_NOT_ACTIVE"); if (Number(row.persistent ?? 0) !== 1 && (Number(row.expires_at) <= now || Number(row.idle_expires_at) <= now)) throw new Error("SESSION_EXPIRED");
    ...
    const session = { state: row.state, persistent: Number(row.persistent ?? 0) === 1, expires_at: Number(row.persistent ?? 0) === 1 ? null : Number(row.expires_at), allowed_operations: scope.allowed_operations, allowed_apps: scope.allowed_apps, capture_scope: scope.capture_scope, delivery_modes: scope.delivery_modes, desktop_epoch: Number(row.desktop_epoch) };
