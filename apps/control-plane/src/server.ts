    ...
    try {
      ...
      if (parts[2] === "internal" && parts[3] === "office") {
        ...
        if (method === "GET" && parts[4] === "operations" && parts.length === 6) {
          const descriptor = operationCatalog({ artifacts: Boolean(options.artifacts), modelTests: Boolean(options.modelTests), modelPreferences: Boolean(options.modelPreferences), onboarding: Boolean(options.onboarding), agentWork: Boolean(options.agentWork) }).find((item) => item.operationId === parts[5]);
        ...
        if (method === "GET" && parts[4] === "operations" && parts.length === 5) {
          return writeJson(response, 200, { catalogVersion: 1, observedAt: new Date().toISOString(), items: operationCatalog({ artifacts: Boolean(options.artifacts), modelTests: Boolean(options.modelTests), modelPreferences: Boolean(options.modelPreferences), onboarding: Boolean(options.onboarding), agentWork: Boolean(options.agentWork) }) }), true;
        }
        if (method === "POST" && parts[4] === "operations" && parts.length === 5) {
          const input = await bodyJson(request); const operationId = String(input.operation_id ?? ""); const descriptor = operationCatalog({ artifacts: Boolean(options.artifacts), modelTests: Boolean(options.modelTests), modelPreferences: Boolean(options.modelPreferences), onboarding: Boolean(options.onboarding), agentWork: Boolean(options.agentWork) }).find((item) => item.operationId === operationId);
          if (!descriptor) throw new Error("OPERATION_NOT_FOUND");
