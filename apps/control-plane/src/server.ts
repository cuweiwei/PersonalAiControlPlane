...
function legacyTaskIntakeEnabled(): boolean { return process.env.PAI_LEGACY_TASK_INTAKE_ENABLED === "true"; }
...
export function createControlPlaneServer(options: Options) {
  ...
  const capabilitySnapshot = async (): Promise<Record<string, unknown>> => {
    ...
    const legacyIntake = legacyTaskIntakeEnabled();
    return { snapshotId: randomUUID(), observedAt: new Date().toISOString(), ttlSeconds: Number(options.settings.get().hermes_brain_capability_ttl_seconds ?? 30), taskContractVersions: legacyIntake ? [1, 2] : [2], legacyTaskIntake: { enabled: legacyIntake, rollbackFlag: "PAI_LEGACY_TASK_INTAKE_ENABLED" }, workerProtocolVersions: [2], features: ["mcp_task_adapter_v1", "attempt_fencing_v1", "durable_result_ack_v1", "workspace_lock_v1", "stop_evidence_v1"], contentHash: safeHash({ items, hermes, legacyIntake }), items, hermes };
  };
    ...
    try {
      ...
      if (parts[2] === "tasks") {
        if (method === "POST" && parts.length === 3) {
          const input = await bodyJson(request); const key = request.headers["idempotency-key"]?.toString(); const defaults = options.settings.taskDefaults();
          if (input.schema_version === 2) { const parsed = parseTaskContractV2Input({ ...input, ...(key ? { idempotency_key: input.idempotency_key ?? key } : {}) }, defaults); parsed.settingsVersion = defaults.settingsVersion; return writeJson(response, 202, options.tasks.delegate(parsed, actor(request))), true; }
          if (!legacyTaskIntakeEnabled()) throw new Error("LEGACY_TASK_INTAKE_DISABLED");
          const requestHash = safeHash(input); if (key) { const prior = options.db.one<Row>("SELECT * FROM operation_receipts WHERE scope = 'tasks-create' AND operation_key = ?", key); if (prior) { if (prior.request_hash !== requestHash) throw new Error("IDEMPOTENCY_CONFLICT"); return writeJson(response, Number(prior.status_code), json(prior.response_json)), true; } }
          const parsed = parseCreateTaskInput(input, defaults); parsed.settingsVersion = defaults.settingsVersion; const created = options.tasks.create(parsed); const result = { task_id: created.id, status: created.status, created_at: created.createdAt, run_id: created.currentRunId }; if (key) options.db.run("INSERT INTO operation_receipts(scope, operation_key, request_hash, status_code, response_json, created_at) VALUES ('tasks-create', ?, ?, 202, ?, ?)", key, requestHash, JSON.stringify(result), Date.now()); return writeJson(response, 202, result), true;
        }
