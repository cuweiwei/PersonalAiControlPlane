...
import { parseDispatchReleasePayload, parseDispatchRulePayload } from "./dispatch/rule-registry.ts";
import { parseDispatchRequest } from "../../../packages/contracts/src/dispatch.ts";
import { parseCreateMemberInput, parseCreateMissionInput, parseCreateRoleInput, parseCreateTaskInput, parseRegistrationInput, parseTaskContractV2Input } from "../../../packages/contracts/src/index.ts";

type Options = { db: ControlPlaneDatabase; tasks: TaskService; workers: WorkerService; coordinator: WorkerCoordinator; missionCoordinator?: MissionCoordinator; artifacts: ArtifactStorage; settings: SettingsService; health: HealthMonitor; events: EventHub; office?: OfficeService; missions?: MissionService; plans?: PlanService; callback?: HermesCallbackDispatcher; modelTests?: ModelTestService; modelPreferences?: ModelPreferenceService; onboarding?: OnboardingService; agentWork?: AgentWorkService; computers?: ComputerSessionService; dispatch?: DispatchService; assetRoot?: string; isReady?: () => boolean };
type Row = Record<string, any>;
const systemEntrySettings: Record<string, string> = { "control-plane": "control_plane_entry_url", hermes: "hermes_entry_url", contexthub: "contexthub_entry_url", "information-radar": "information_radar_entry_url" };
function projectSystemEntries(items: Record<string, unknown>[], values: Record<string, unknown>): Record<string, unknown>[] { return items.map((item) => { const setting = systemEntrySettings[String(item.id)]; return setting && values[setting] ? { ...item, entryUrl: values[setting] } : item; }); }
...
export function createControlPlaneServer(options: Options) {
  ...
  const capabilitySnapshot = async (): Promise<Record<string, unknown>> => {
    ...
    return { snapshotId: randomUUID(), observedAt: new Date().toISOString(), ttlSeconds: Number(options.settings.get().hermes_brain_capability_ttl_seconds ?? 30), taskContractVersions: [1, 2], workerProtocolVersions: [2], features: ["mcp_task_adapter_v1", "attempt_fencing_v1", "durable_result_ack_v1", "workspace_lock_v1", "stop_evidence_v1", "computer_session_v1"], contentHash: safeHash({ items, hermes }), items, hermes };
  };
  const queryCapabilities = async (input: Record<string, unknown>): Promise<Record<string, unknown>> => {
    ...
    for (const row of rows) {
      const item = grouped.get(String(row.worker_id)) ?? { worker_id: row.worker_id, name: row.name, status: row.status, capabilities: [], verification_refs: [] };
      const descriptor = json(row.descriptor_json, {});
      (item.capabilities as Row[]).push({ id: row.capability, runtime: row.runtime, runtime_version: row.runtime_version, contract_version: Number(row.contract_version ?? 1), evidence_state: row.evidence_state ?? "ADVERTISED", grant_status: row.grant_status ?? "DISCOVERED", descriptor });
      if (row.verification_ref) (item.verification_refs as string[]).push(String(row.verification_ref));
      grouped.set(String(row.worker_id), item);
    }
  ...
  const api = async (request: IncomingMessage, response: ServerResponse): Promise<boolean> => {
    ...
    try {
      ...
      if (parts[2] === "capabilities" && method === "POST" && parts[3] === "query" && parts.length === 4) {
        return writeJson(response, 200, await queryCapabilities(await bodyJson(request))), true;
      }
      if (parts[2] === "dispatch" && parts[3] === "effective" && method === "GET" && parts.length === 4) {
        const effective = options.settings.getEffective();
        return writeJson(response, 200, {
          schema_version: 1,
          settings_version: effective.version,
          enabled: effective.values.dispatch_enabled === true,
          semantic_enabled: effective.values.dispatch_semantic_enabled === true,
          routing_budget_ms: Number(effective.values.dispatch_routing_budget_ms),
          source: "control_plane_settings",
          observed_at: new Date().toISOString(),
        }), true;
      }
      if (parts[2] === "dispatch" && options.dispatch) {
        if (method === "POST" && parts[3] === "prepare" && parts.length === 4) {
          return writeJson(response, 200, await options.dispatch.prepare(parseDispatchRequest(await bodyJson(request)), actor(request))), true;
        }
        if (method === "POST" && parts[3] === "proposals" && parts[4] && parts[5] === "commit" && parts.length === 6) {
          const input = await bodyJson(request);
          const operationKey = String(input.operation_key ?? ""); const actionHash = String(input.action_hash ?? "");
          if (!operationKey || !actionHash || !Number.isInteger(input.session_revision) || typeof input.ownership_ref !== "string") throw new Error("INVALID_DISPATCH_COMMIT");
          return writeJson(response, 202, options.dispatch.commit(parts[4], { operationKey, actionHash, sessionRevision: Number(input.session_revision), ownershipRef: input.ownership_ref }, actor(request))), true;
        }
        if (method === "POST" && parts[3] === "proposals" && parts[4] && parts[5] === "resolve-to-hermes" && parts.length === 6) {
          const input = await bodyJson(request); const operationKey = String(input.operation_key ?? ""); const actionHash = String(input.action_hash ?? "");
          if (!operationKey || !actionHash) throw new Error("INVALID_DISPATCH_RESOLVE");
          return writeJson(response, 200, options.dispatch.resolveToHermes(parts[4], { operationKey, actionHash }, actor(request))), true;
        }
        if (method === "GET" && parts[3] === "requests" && parts[4] && parts.length === 5) {
          const result = options.dispatch.getRequest(actor(request), parts[4]); if (!result) throw new Error("DISPATCH_REQUEST_NOT_FOUND"); return writeJson(response, 200, result), true;
        }
        if (method === "POST" && parts[3] === "observations" && parts.length === 4) {
          const input = await bodyJson(request); const actual = Array.isArray(input.actual_actions) ? input.actual_actions : [];
          if (!input.source_event_key || actual.length !== 1) throw new Error("INVALID_DISPATCH_OBSERVATION");
          return writeJson(response, 202, options.dispatch.observe({ sourceEventKey: String(input.source_event_key), ingressKey: input.ingress_key ? String(input.ingress_key) : undefined, requestId: input.request_id ? String(input.request_id) : undefined, actualActions: actual as any, outcome: String(input.outcome ?? "UNKNOWN"), evidenceRef: input.evidence_ref ? String(input.evidence_ref) : undefined, occurredAt: input.occurred_at ? Date.parse(String(input.occurred_at)) : undefined })), true;
        }
        if (method === "GET" && parts[3] === "rules" && parts.length === 4) return writeJson(response, 200, { items: options.dispatch.rules() }), true;
        if (method === "POST" && parts[3] === "rules" && parts.length === 4) return writeJson(response, 201, options.dispatch.createCandidate(parseDispatchRulePayload(await bodyJson(request)), actor(request))), true;
        if (method === "POST" && parts[3] === "rules" && parts[4] && parts[5] === "transitions" && parts.length === 6) {
          const input = await bodyJson(request); return writeJson(response, 200, options.dispatch.transition(parts[4], Number(input.revision), String(input.to).toUpperCase(), Number(input.expected_state_revision), actor(request), String(input.reason ?? "MANUAL"), input.evidence_ref ? String(input.evidence_ref) : undefined)), true;
        }
        if (method === "POST" && parts[3] === "releases" && parts.length === 4) return writeJson(response, 201, options.dispatch.createRelease(parseDispatchReleasePayload(await bodyJson(request)), actor(request))), true;
        if (method === "PUT" && parts[3] === "active-release" && parts.length === 4) { const input = await bodyJson(request); return writeJson(response, 200, options.dispatch.activateRelease(String(input.release_id), Number(input.expected_revision), actor(request))), true; }
      }
      if (parts[2] === "mcp" && method === "POST" && parts.length === 3) {
        if (!(await internalPeerAllowed(request))) throw new Error("INTERNAL_ROUTE_FORBIDDEN");
        const rpc = await bodyJson(request); const id = rpc.id ?? null; const rawName = String(rpc.method ?? ""); const name = ({ "control.capabilities.query": "find_capabilities", "control.tasks.delegate": "delegate_task", "control.tasks.get": "get_task_status", "control.tasks.result": "get_task_result", "control.tasks.wait": "wait_task", "control.tasks.cancel": "cancel_task", "control.tasks.retry": "retry_task" } as Record<string, string>)[rawName] ?? rawName; const parameters = rpc.params && typeof rpc.params === "object" && !Array.isArray(rpc.params) ? rpc.params as Row : {};
        try {
          let result: unknown;
          if (name === "find_capabilities") result = await queryCapabilities(parameters);
          else if (name === "delegate_task") { const parsed = parseTaskContractV2Input({ ...parameters, schema_version: parameters.schema_version ?? 2, idempotency_key: parameters.idempotency_key ?? request.headers["idempotency-key"]?.toString() }, options.settings.taskDefaults()); result = options.tasks.delegate(parsed, actor(request)); }
          else if (name === "get_task_status") { result = options.tasks.detail(String(parameters.task_id ?? "")); if (!result) throw new Error("TASK_NOT_FOUND"); }
          else if (name === "get_task_result") { result = options.tasks.resultView(String(parameters.task_id ?? ""), parameters.run_id ? String(parameters.run_id) : undefined); if (!result) throw new Error("TASK_NOT_FOUND"); }
          else if (name === "wait_task") { result = await options.tasks.waitTask(String(parameters.task_id ?? ""), Number(parameters.after_revision ?? 0), Number(parameters.timeout_ms ?? 30_000)); if (!result) throw new Error("TASK_NOT_FOUND"); }
          else if (name === "cancel_task") { result = options.tasks.cancelWithOptions(String(parameters.task_id ?? ""), { expectedRevision: parameters.expected_revision === undefined ? undefined : Number(parameters.expected_revision), idempotencyKey: String(parameters.idempotency_key ?? request.headers["idempotency-key"]?.toString() ?? ""), principal: actor(request) }); if (!result) throw new Error("TASK_NOT_FOUND"); }
          else if (name === "retry_task") { result = options.tasks.retryWithOptions(String(parameters.task_id ?? ""), { expectedRunId: parameters.expected_run_id ? String(parameters.expected_run_id) : undefined, expectedRevision: parameters.expected_revision === undefined ? undefined : Number(parameters.expected_revision), idempotencyKey: String(parameters.idempotency_key ?? request.headers["idempotency-key"]?.toString() ?? "") }); if (!result) throw new Error("TASK_NOT_FOUND"); }
          else throw new Error("MCP_METHOD_NOT_FOUND");
          return writeJson(response, 200, { jsonrpc: "2.0", id, result }), true;
        } catch (error) { const code = error instanceof Error ? error.message : "INTERNAL_ERROR"; return writeJson(response, 200, { jsonrpc: "2.0", id, error: { code, message: code } }), true; }
      }
      ...
      if (parts[2] === "missions") {
        ...
        if (method === "POST" && parts[4] === "memory-requests" && parts.length === 5 && options.agentWork) return writeJson(response, 503, options.agentWork.memoryRequest(missionId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
      ...
      if (options.agentWork && parts[2] === "skills") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.agentWork.listSkills(q.get("office_id") ?? undefined), observedAt: new Date().toISOString() }), true;
        if (method === "POST" && parts.length === 3) return writeJson(response, 201, options.agentWork.createSkillDraft(await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        const skillId = parts[3]; if (!skillId) throw new Error("SKILL_NOT_FOUND");
        if (method === "GET" && parts.length === 4) { const skill = options.agentWork.getSkill(skillId); if (!skill) throw new Error("SKILL_NOT_FOUND"); return writeJson(response, 200, skill), true; }
        if (method === "POST" && parts[4] === "versions" && parts.length === 5) return writeJson(response, 201, options.agentWork.createSkillVersion(skillId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        const version = parts[5] ? Number(parts[5]) : undefined;
        if (method === "GET" && parts[4] === "versions" && version !== undefined && parts.length === 6) { const item = options.agentWork.getSkillVersion(skillId, version); if (!item) throw new Error("SKILL_VERSION_NOT_FOUND"); return writeJson(response, 200, item), true; }
        if (method === "POST" && parts[4] === "versions" && version !== undefined && parts[6] === "validate") return writeJson(response, 202, options.agentWork.validateSkill(skillId, version, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        if (method === "POST" && parts[4] === "activate" && parts.length === 5) return writeJson(response, 200, options.agentWork.activateSkill(skillId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        if (method === "POST" && parts[4] === "runs" && parts.length === 5) return writeJson(response, 202, options.agentWork.runSkill(skillId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
      }
      if (options.agentWork && parts[2] === "routine-bindings") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.agentWork.listRoutines(q.get("office_id") ?? undefined), observedAt: new Date().toISOString() }), true;
        if (method === "POST" && parts.length === 3) return writeJson(response, 202, options.agentWork.createRoutine(await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        const bindingId = parts[3]; if (!bindingId) throw new Error("ROUTINE_NOT_FOUND");
        if (method === "GET" && parts.length === 4) { const binding = options.agentWork.getRoutine(bindingId); if (!binding) throw new Error("ROUTINE_NOT_FOUND"); return writeJson(response, 200, binding), true; }
        if (method === "GET" && parts[4] === "occurrences") { const binding = options.agentWork.getRoutine(bindingId); if (!binding) throw new Error("ROUTINE_NOT_FOUND"); return writeJson(response, 200, { items: (binding.occurrences ?? []) }), true; }
      }
      if (options.agentWork && parts[2] === "routine-occurrences" && method === "POST" && parts.length === 3) { if (!(await internalPeerAllowed(request))) throw new Error("INTERNAL_ROUTE_FORBIDDEN"); return writeJson(response, 202, options.agentWork.receiveOccurrence(await bodyJson(request), actor(request))), true; }
      if (options.agentWork && parts[2] === "goals") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.agentWork.listGoals(q.get("office_id") ?? undefined), observedAt: new Date().toISOString() }), true;
        if (method === "POST" && parts.length === 3) return writeJson(response, 201, options.agentWork.createGoal(await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        const goalId = parts[3]; if (!goalId) throw new Error("GOAL_NOT_FOUND");
        if (method === "GET" && parts.length === 4) { const goal = options.agentWork.getGoal(goalId); if (!goal) throw new Error("GOAL_NOT_FOUND"); return writeJson(response, 200, goal), true; }
        if (method === "POST" && parts[4] === "commands" && parts.length === 5) return writeJson(response, 202, options.agentWork.goalCommand(goalId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        if (method === "POST" && parts[4] === "budget" && parts.length === 5) return writeJson(response, 202, options.agentWork.reserveBudget(goalId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
      }
      if (options.agentWork && parts[2] === "attention") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.agentWork.listAttention({ state: q.get("state") ?? undefined, severity: q.get("severity") ?? undefined, limit: q.has("limit") ? Number(q.get("limit")) : undefined }), observedAt: new Date().toISOString() }), true;
        const attentionId = parts[3]; if (!attentionId) throw new Error("ATTENTION_NOT_FOUND");
        if (method === "POST" && parts[4] === "commands" && parts.length === 5) return writeJson(response, 200, options.agentWork.attentionCommand(attentionId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
      }
      if (options.agentWork && parts[2] === "artifacts" && parts[4] === "view" && method === "GET") return writeJson(response, 200, options.agentWork.artifactView(parts[3] ?? "")), true;
      if (options.agentWork && parts[2] === "browser-sessions") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.agentWork.listBrowserSessions(), observedAt: new Date().toISOString() }), true;
        const sessionId = parts[3]; if (!sessionId) throw new Error("BROWSER_SESSION_NOT_FOUND");
        if (method === "GET" && parts.length === 4) { const session = options.agentWork.getBrowserSession(sessionId); if (!session) throw new Error("BROWSER_SESSION_NOT_FOUND"); return writeJson(response, 200, session), true; }
        if (method === "POST" && parts[4] === "commands" && parts.length === 5) return writeJson(response, 202, options.agentWork.browserCommand(sessionId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
      }
      if (options.agentWork && parts[2] === "teaching-sessions") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.agentWork.listTeachingSessions(), observedAt: new Date().toISOString() }), true;
        if (method === "POST" && parts.length === 3) return writeJson(response, 201, options.agentWork.createTeachingSession(await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
        const teachingId = parts[3]; if (!teachingId) throw new Error("TEACHING_SESSION_NOT_FOUND");
        if (method === "POST" && parts[4] === "commands" && parts.length === 5) return writeJson(response, 202, options.agentWork.teachingCommand(teachingId, await bodyJson(request), actor(request), request.headers["idempotency-key"]?.toString() ?? "")), true;
      }
      if (options.agentWork && parts[2] === "agent-work") {
        if (method === "GET" && parts[3] === "capabilities" && parts.length === 4) return writeJson(response, 200, options.agentWork.capabilities()), true;
        if (method === "GET" && parts[3] === "events" && parts.length === 4) return writeJson(response, 200, options.agentWork.eventsPage(Number(q.get("after_seq") ?? 0), Number(q.get("limit") ?? 100))), true;
        if (method === "GET" && parts[3] === "operations" && parts[4]) { const operation = options.agentWork.getOperation(parts[4]); if (!operation) throw new Error("OPERATION_NOT_FOUND"); return writeJson(response, 200, operation), true; }
        if (method === "POST" && parts[3] === "operations" && parts.length === 4) { const input = await bodyJson(request); const kind = String(input.operation_kind ?? ""); const parameters = input.parameters && typeof input.parameters === "object" && !Array.isArray(input.parameters) ? input.parameters as Record<string, unknown> : input; const key = request.headers["idempotency-key"]?.toString() ?? ""; if (kind === "control.skills.draft_create") return writeJson(response, 202, options.agentWork.createSkillDraft(parameters, actor(request), key)), true; if (kind === "control.skills.validate") return writeJson(response, 202, options.agentWork.validateSkill(String(parameters.skill_id ?? ""), Number(parameters.version), parameters, actor(request), key)), true; if (kind === "control.skills.activate") return writeJson(response, 200, options.agentWork.activateSkill(String(parameters.skill_id ?? ""), parameters, actor(request), key)), true; if (kind === "control.skills.run") return writeJson(response, 202, options.agentWork.runSkill(String(parameters.skill_id ?? ""), parameters, actor(request), key)), true; if (kind === "control.routines.create") return writeJson(response, 202, options.agentWork.createRoutine(parameters, actor(request), key)), true; if (kind === "control.goals.create") return writeJson(response, 201, options.agentWork.createGoal(parameters, actor(request), key)), true; if (kind === "control.goals.command") return writeJson(response, 202, options.agentWork.goalCommand(String(parameters.goal_id ?? ""), parameters, actor(request), key)), true; if (kind === "control.attention.command") return writeJson(response, 200, options.agentWork.attentionCommand(String(parameters.attention_id ?? ""), parameters, actor(request), key)), true; throw new Error("OPERATION_NOT_IMPLEMENTED"); }
      }
      if (options.agentWork && parts[2] === "internal" && parts[3] === "agent-work") {
        if (!(await internalPeerAllowed(request))) throw new Error("INTERNAL_ROUTE_FORBIDDEN");
        if (method === "POST" && parts[4] === "receipts" && parts.length === 5) return writeJson(response, 200, options.agentWork.applyRoutineReceipt(await bodyJson(request))), true;
      }
      if (parts[2] === "internal" && parts[3] === "office") {
        ...
        if (method === "GET" && parts[4] === "operations" && parts.length === 6) {
          const descriptor = operationCatalog({ artifacts: Boolean(options.artifacts), modelTests: Boolean(options.modelTests), modelPreferences: Boolean(options.modelPreferences), onboarding: Boolean(options.onboarding), agentWork: Boolean(options.agentWork), dispatch: Boolean(options.dispatch) }).find((item) => item.operationId === parts[5]);
        ...
        if (method === "GET" && parts[4] === "operations" && parts.length === 5) {
          return writeJson(response, 200, { catalogVersion: 1, observedAt: new Date().toISOString(), items: operationCatalog({ artifacts: Boolean(options.artifacts), modelTests: Boolean(options.modelTests), modelPreferences: Boolean(options.modelPreferences), onboarding: Boolean(options.onboarding), agentWork: Boolean(options.agentWork), dispatch: Boolean(options.dispatch) }) }), true;
        }
        if (method === "POST" && parts[4] === "operations" && parts.length === 5) {
          const input = await bodyJson(request); const operationId = String(input.operation_id ?? ""); const descriptor = operationCatalog({ artifacts: Boolean(options.artifacts), modelTests: Boolean(options.modelTests), modelPreferences: Boolean(options.modelPreferences), onboarding: Boolean(options.onboarding), agentWork: Boolean(options.agentWork), dispatch: Boolean(options.dispatch) }).find((item) => item.operationId === operationId);
          if (!descriptor) throw new Error("OPERATION_NOT_FOUND");
      ...
      if (parts[2] === "tasks") {
        if (options.computers && method === "POST" && parts.length === 3) {
          const input = await bodyJson(request);
          if (input.task_type === "computer.use") {
            const key = request.headers["idempotency-key"]?.toString(); if (!key) throw new Error("MISSING_IDEMPOTENCY_KEY");
            const requestHash = safeHash(input);
            const prior = options.db.one<Row>("SELECT * FROM operation_receipts WHERE scope = 'tasks-create' AND operation_key = ?", key);
            if (prior) { if (prior.request_hash !== requestHash) throw new Error("IDEMPOTENCY_CONFLICT"); return writeJson(response, Number(prior.status_code), json(prior.response_json)), true; }
            const defaults = options.settings.taskDefaults();
            const parsed = input.schema_version === 2 ? parseTaskContractV2Input({ ...input, idempotency_key: input.idempotency_key ?? key }, defaults) : parseCreateTaskInput(input, defaults);
            const prepared = options.computers.prepareTask(parsed, actor(request));
            const created = options.tasks.create(prepared);
            options.computers.recordOperation(String((prepared.payload as Row).session_id), String(created.id), prepared.payload as Row);
            const result = { task_id: created.id, status: created.status, created_at: created.createdAt, run_id: created.currentRunId };
            options.db.run("INSERT INTO operation_receipts(scope, operation_key, request_hash, status_code, response_json, created_at, retain_until) VALUES ('tasks-create', ?, ?, 202, ?, ?, ?)", key, requestHash, JSON.stringify(result), Date.now(), Date.now() + 86_400_000);
            return writeJson(response, 202, result), true;
          }
        }
        if (method === "POST" && parts.length === 3) { const input = await bodyJson(request); const key = request.headers["idempotency-key"]?.toString(); const defaults = options.settings.taskDefaults(); if (input.schema_version === 2) { const parsed = parseTaskContractV2Input({ ...input, ...(key ? { idempotency_key: input.idempotency_key ?? key } : {}) }, defaults); parsed.settingsVersion = defaults.settingsVersion; return writeJson(response, 202, options.tasks.delegate(parsed, actor(request))), true; } const requestHash = safeHash(input); if (key) { const prior = options.db.one<Row>("SELECT * FROM operation_receipts WHERE scope = 'tasks-create' AND operation_key = ?", key); if (prior) { if (prior.request_hash !== requestHash) throw new Error("IDEMPOTENCY_CONFLICT"); return writeJson(response, Number(prior.status_code), json(prior.response_json)), true; } } const parsed = parseCreateTaskInput(input, defaults); parsed.settingsVersion = defaults.settingsVersion; const created = options.tasks.create(parsed); const result = { task_id: created.id, status: created.status, created_at: created.createdAt, run_id: created.currentRunId }; if (key) options.db.run("INSERT INTO operation_receipts(scope, operation_key, request_hash, status_code, response_json, created_at) VALUES ('tasks-create', ?, ?, 202, ?, ?)", key, requestHash, JSON.stringify(result), Date.now()); return writeJson(response, 202, result), true; }
        ...
        if (method === "POST" && parts[4] === "cancel") { const input = await bodyJson(request); const key = request.headers["idempotency-key"]?.toString(); const current = options.tasks.get(taskId); if (!current) throw new Error("TASK_NOT_FOUND"); if ((current as Row).executionSemantics === "platform_v2" && !key) throw new Error("MISSING_IDEMPOTENCY_KEY"); const cancelled = options.tasks.cancelWithOptions(taskId, { expectedRevision: typeof input.expected_revision === "number" ? input.expected_revision : typeof input.expected_task_revision === "number" ? input.expected_task_revision : undefined, idempotencyKey: key, principal: actor(request) }); if (!cancelled) throw new Error("TASK_NOT_FOUND"); const attemptId = String(cancelled.currentAttemptId ?? ""); const workerId = attemptId ? String(options.db.one<Row>("SELECT worker_id FROM task_attempts WHERE id = ?", attemptId)?.worker_id ?? "") : ""; if (workerId && attemptId) options.coordinator.cancel(workerId, taskId, attemptId); return writeJson(response, 202, cancelled), true; }
        ...
        if (method === "GET" && parts[4] === "results") { const result = options.tasks.resultView(taskId, q.get("run_id") ?? undefined); if (!result) throw new Error("TASK_NOT_FOUND"); return writeJson(response, 200, result), true; }
        if (method === "GET" && parts[4] === "wait") { const task = options.tasks.get(taskId); if (!task) throw new Error("TASK_NOT_FOUND"); const waited = await options.tasks.waitTask(taskId, Number(q.get("after_revision") ?? q.get("afterRevision") ?? 0), Number(q.get("timeout_ms") ?? q.get("timeoutMs") ?? 30_000)); return writeJson(response, 200, { task: waited, revision: (waited as Row | undefined)?.revision ?? null, timed_out: Number((waited as Row | undefined)?.revision ?? 0) <= Number(q.get("after_revision") ?? q.get("afterRevision") ?? 0) }), true; }
      ...
      if (options.computers && parts[2] === "computers") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.computers.listComputers(), observedAt: new Date().toISOString() }), true;
      }
      if (options.computers && parts[2] === "computer-sessions") {
        if (method === "GET" && parts.length === 3) return writeJson(response, 200, { items: options.computers.listSessions(actor(request)) }), true;
        if (method === "POST" && parts.length === 3) {
          const key = request.headers["idempotency-key"]?.toString() ?? "";
          const input = await bodyJson(request);
          const result = options.computers.create({ principal: actor(request), workRef: input.work_ref as Record<string, unknown> | null | undefined, workerId: String(input.worker_id ?? ""), desktopId: input.desktop_id === undefined ? undefined : String(input.desktop_id), desktopKind: input.desktop_kind as "desktop" | "window" | undefined, displayId: input.display_id === undefined ? undefined : String(input.display_id), allowedOperations: Array.isArray(input.allowed_operations) ? input.allowed_operations.map(String) : undefined, allowedApps: Array.isArray(input.allowed_apps) ? input.allowed_apps.map(String) : undefined, captureScope: input.capture_scope as "window" | "desktop" | undefined, deliveryModes: Array.isArray(input.delivery_modes) ? input.delivery_modes.map(String) : undefined, model: input.model && typeof input.model === "object" ? { provider: String((input.model as Row).provider ?? ""), id: String((input.model as Row).id ?? (input.model as Row).model_id ?? ""), dataPolicy: String((input.model as Row).data_policy ?? (input.model as Row).dataPolicy ?? "") || undefined } : undefined, maxActions: input.max_actions === undefined ? undefined : Number(input.max_actions) }, key); return writeJson(response, 201, result), true;
        }
        const sessionId = parts[3]; if (!sessionId) throw new Error("COMPUTER_SESSION_NOT_FOUND");
        if (parts[4] === "observations" && parts[5]) {
          const observation = options.db.one<Row>("SELECT o.*, a.media_type, a.size_bytes, a.sha256, a.storage_path, a.display_filename, a.filename, a.storage_state FROM computer_observations o LEFT JOIN artifacts a ON a.id = o.artifact_id WHERE o.id = ? AND o.session_id = ?", parts[5], sessionId);
          if (!observation) throw new Error("OBSERVATION_NOT_FOUND");
          const owner = options.db.one<Row>("SELECT principal FROM computer_sessions WHERE id = ?", sessionId); if (!owner || String(owner.principal ?? "owner") !== actor(request)) throw new Error("SESSION_ACCESS_DENIED");
          if (Number(observation.expires_at) <= Date.now() || String(observation.storage_state ?? "AVAILABLE") !== "AVAILABLE") throw new Error("ARTIFACT_EXPIRED");
          if (method === "GET" && parts.length === 6) return writeJson(response, 200, { id: observation.id, sessionId, sequence: Number(observation.sequence), target: json(observation.target_json), metadata: json(observation.metadata_json), artifact: observation.artifact_id ? { id: observation.artifact_id, mediaType: observation.media_type, sizeBytes: Number(observation.size_bytes), sha256: observation.sha256, filename: observation.display_filename ?? observation.filename } : null, expiresAt: new Date(Number(observation.expires_at)).toISOString() }), true;
          if (method === "GET" && parts.length === 7 && parts[6] === "download") {
            if (!observation.artifact_id || !observation.storage_path || !options.artifacts.exists(observation.storage_path)) throw new Error("ARTIFACT_MISSING");
            response.writeHead(200, { "content-type": observation.media_type ?? "application/octet-stream", "content-length": observation.size_bytes, "content-disposition": `attachment; filename*=UTF-8''${encodeURIComponent(String(observation.display_filename ?? observation.filename ?? "observation.png"))}`, "cache-control": "no-store" });
            options.artifacts.stream(observation.storage_path).pipe(response); return true;
          }
        }
        if (method === "POST" && parts[4] === "operations" && parts[5] && parts[6] === "reconcile" && parts.length === 7) {
          const input = await bodyJson(request); const sequence = Number(parts[5]); if (!Number.isInteger(sequence) || sequence < 1) throw new Error("INVALID_OPERATION_SEQUENCE"); return writeJson(response, 200, options.computers.reconcileOperation(sessionId, sequence, String(input.observation_id ?? input.observationId ?? ""), stepUpActor(request), Date.now())), true;
        }
        if (method === "GET" && parts.length === 4) { const session = options.computers.get(sessionId); if (!session) throw new Error("COMPUTER_SESSION_NOT_FOUND"); if (String(session.principal ?? "owner") !== actor(request)) throw new Error("SESSION_ACCESS_DENIED"); return writeJson(response, 200, session), true; }
        if (method === "DELETE" && parts.length === 4) { const input = await bodyJson(request); const expected = Number(input.expected_revision ?? input.expectedRevision); if (!Number.isInteger(expected)) throw new Error("MISSING_EXPECTED_REVISION"); return writeJson(response, 202, options.computers.deleteSession(sessionId, expected, stepUpActor(request), Date.now())), true; }
        if (method === "POST" && parts[4] === "approve" && parts.length === 5) { const input = await bodyJson(request); return writeJson(response, 200, options.computers.approve(sessionId, String(input.scope_hash ?? input.scopeHash ?? ""), stepUpActor(request), Date.now())), true; }
        if (method === "POST" && parts[4] === "control" && parts.length === 5) { const input = await bodyJson(request); const action = String(input.action ?? "") as "pause" | "resume" | "close" | "revoke"; if (!["pause", "resume", "close", "revoke"].includes(action)) throw new Error("INVALID_SESSION_ACTION"); const expected = Number(input.expected_revision ?? input.expectedRevision); if (!Number.isInteger(expected)) throw new Error("MISSING_EXPECTED_REVISION"); return writeJson(response, 202, options.computers.control(sessionId, action, expected, stepUpActor(request), Date.now())), true; }
      }
      if (parts[2] === "workers") {
      ...
      if (parts[2] === "systems" && method === "GET") { const values = options.settings.get(); const items = projectSystemEntries(options.health.list(Boolean(options.isReady?.() ?? true)), values); return writeJson(response, 200, { items }), true; }
      if (parts[2] === "settings") {
      ...
      if (parts[2] === "dashboard" && method === "GET") {
        ...
        const values = options.settings.get(); const systems = projectSystemEntries(options.health.list(Boolean(options.isReady?.() ?? true)), values); return writeJson(response, 200, { attention: { items: attention, observedAt: new Date().toISOString() }, running: { items: (active.countsByStatus as Row).RUNNING ?? 0, observedAt: active.observedAt }, latestResults: { items: latest.items, observedAt: new Date().toISOString() }, recentTasks: { items: recent.items, observedAt: recent.observedAt }, summary24h: { ...options.tasks.summary({ purpose: "USER", createdFrom: since }), observedAt: new Date().toISOString() }, systems: { items: systems, observedAt: new Date().toISOString() } }), true;
      }
      if (parts[2] === "artifacts" && parts[3]) {
        ...
        if (parts[4] === "download" && method === "GET") { const observation = options.db.one<Row>("SELECT expires_at FROM computer_observations WHERE artifact_id = ?", artifact.id); if (observation && Number(observation.expires_at) <= Date.now()) throw new Error("ARTIFACT_EXPIRED"); if (["EXPIRED", "PURGING"].includes(String(artifact.storage_state ?? "AVAILABLE"))) throw new Error("ARTIFACT_EXPIRED"); if (!options.artifacts.exists(artifact.storage_path)) throw new Error("ARTIFACT_MISSING"); response.writeHead(200, { "content-type": artifact.media_type ?? "application/octet-stream", "content-length": artifact.size_bytes, "content-disposition": `attachment; filename*=UTF-8''${encodeURIComponent(String(artifact.display_filename ?? artifact.filename))}`, "cache-control": "no-store" }); options.artifacts.stream(artifact.storage_path).pipe(response); return true; }
      ...
      if (parts[2] === "worker" && parts[3] === "artifacts" && method === "GET") { const worker = authWorker(request, options.workers); const artifactId = parts[4]; const row = options.db.one<Row>("SELECT a.*, o.expires_at AS observation_expires_at FROM artifacts a JOIN task_artifacts ta ON ta.artifact_id = a.id JOIN task_attempts at ON at.task_id = ta.task_id LEFT JOIN computer_observations o ON o.artifact_id = a.id WHERE a.id = ? AND at.worker_id = ?", artifactId, worker.id); if (!row || (row.observation_expires_at && Number(row.observation_expires_at) <= Date.now()) || ["EXPIRED", "PURGING"].includes(String(row.storage_state ?? "AVAILABLE")) || !options.artifacts.exists(row.storage_path)) throw new Error(row?.observation_expires_at ? "ARTIFACT_EXPIRED" : "ARTIFACT_NOT_FOUND"); response.writeHead(200, { "content-type": row.media_type ?? "application/octet-stream", "content-length": row.size_bytes, "content-disposition": `attachment; filename="${basename(row.filename)}"`, "cache-control": "no-store" }); options.artifacts.stream(row.storage_path).pipe(response); return true; }
      ...
      const status = ["TASK_NOT_FOUND", "WORKER_NOT_FOUND", "REGISTRATION_NOT_FOUND", "ARTIFACT_NOT_FOUND", "CAPABILITY_NOT_FOUND", "PREFERENCE_NOT_FOUND", "ONBOARDING_NOT_FOUND", "MODEL_TEST_NOT_FOUND", "DELIVERY_NOT_FOUND", "OFFICE_NOT_FOUND", "ROLE_NOT_FOUND", "MISSION_NOT_FOUND", "COMMAND_NOT_FOUND", "OPERATION_NOT_FOUND", "SKILL_NOT_FOUND", "SKILL_VERSION_NOT_FOUND", "ROUTINE_NOT_FOUND", "GOAL_NOT_FOUND", "MILESTONE_NOT_FOUND", "ATTENTION_NOT_FOUND", "BROWSER_SESSION_NOT_FOUND", "TEACHING_SESSION_NOT_FOUND", "BUDGET_ACCOUNT_NOT_FOUND", "BUDGET_RESERVATION_NOT_FOUND", "COMPUTER_SESSION_NOT_FOUND", "OBSERVATION_NOT_FOUND", "DISPATCH_PROPOSAL_NOT_FOUND", "DISPATCH_REQUEST_NOT_FOUND", "DISPATCH_RULE_NOT_FOUND", "DISPATCH_RELEASE_NOT_FOUND", "DISPATCH_SHADOW_NOT_FOUND", "DISPATCH_OBSERVATION_NOT_FOUND"] .includes(code) ? 404 : code === "INVALID_WORKER_TOKEN" ? 401 : code === "REGISTRATION_DISABLED" || code === "STEP_UP_REQUIRED" || code === "INTERNAL_ROUTE_FORBIDDEN" || code === "SESSION_ACCESS_DENIED" ? 403 : ["INVALID_TASK_STATE", "INVALID_REGISTRATION_STATE", "WORKER_BUSY", "REGISTRATION_ALREADY_FINALIZED", "IDEMPOTENCY_CONFLICT", "TASK_CHANGED", "CURSOR_STALE", "SETTING_OVERRIDDEN", "WORKER_PREFERENCES_CHANGED", "SETTINGS_CHANGED", "PREFERENCE_CHANGED", "INPUT_ARTIFACT_EXPIRED", "RECEIPT_CONFLICT", "TEMPLATE_CHANGED", "WORKSPACE_MISSING", "ARTIFACT_CONTENT_CONFLICT", "RESULT_ARTIFACT_NOT_READY", "ARTIFACT_MISSING", "REVISION_CONFLICT", "PLAN_ALREADY_COMMITTED", "RESULT_CONFLICT", "SEAT_ALREADY_ASSIGNED", "ADMISSION_DEFERRED", "STALE_EXECUTION", "STALE_DECISION", "LIMIT_WAIT_OWNER", "FINALIZATION_INCOMPLETE", "RECOVERY_RECONCILIATION_REQUIRED", "RECOVERY_MODE", "COMMAND_RETRY_NOT_AVAILABLE", "MISSION_TERMINAL", "MISSION_NOT_TERMINAL", "SIDE_EFFECT_IN_PROGRESS", "OPERATION_NOT_IMPLEMENTED", "SKILL_NOT_READY", "VALIDATION_INCOMPLETE", "BINDING_REVISION_STALE", "GOAL_NOT_ACTIVE", "BUDGET_EXHAUSTED", "EXECUTION_UNKNOWN", "FEATURE_DISABLED", "CAPABILITY_UNAVAILABLE", "BUDGET_RESERVATION_NOT_FOUND", "INVALID_GOAL_STATE", "MILESTONES_INCOMPLETE", "DESKTOP_BUSY", "SESSION_CHANGED", "DESKTOP_CHANGED", "SESSION_NOT_ACTIVE", "SESSION_SCOPE_DENIED", "APP_SCOPE_DENIED", "COMPUTER_CAPABILITY_UNAVAILABLE", "CAPTURE_SCOPE_MISMATCH", "CAPTURE_SCOPE_DENIED", "OBSERVATION_STALE", "OBSERVATION_REQUIRED", "OBSERVATION_NOT_FOUND", "INVALID_OPERATION_SCOPE", "OPERATION_SEQUENCE_CONFLICT", "OPERATION_ALREADY_EXISTS", "INVALID_SESSION_STATE", "INVALID_SESSION_ACTION", "STOP_UNCONFIRMED", "SESSION_EXPIRED", "WINDOW_REF_INVALID", "DRIVER_ENDPOINT_REQUIRED", "DRIVER_SCHEMA_UNSUPPORTED", "APPROVAL_REQUIRED", "CUA_DIRECTION_REQUIRED", "CUA_KEYS_REQUIRED", "COMPUTER_OBSERVATION_LIMIT", "DISPATCH_REQUEST_CONFLICT", "DISPATCH_ACTION_CONFLICT", "DISPATCH_COMMIT_CONFLICT", "DISPATCH_PROPOSAL_EXPIRED", "DISPATCH_PROPOSAL_RELEASED", "DISPATCH_PROPOSAL_NOT_COMMITTABLE", "DISPATCH_PROPOSAL_REVOKED", "DISPATCH_ALREADY_ACCEPTED", "DISPATCH_PROPOSAL_NOT_RESOLVABLE", "DISPATCH_SESSION_CHANGED", "DISPATCH_RULE_REVISION_CONFLICT", "DISPATCH_RULE_STATE_CHANGED", "DISPATCH_RULE_INVALID_TRANSITION", "DISPATCH_RELEASE_CHANGED", "DISPATCH_OPERATION_UNAVAILABLE", "DISPATCH_RULE_RISK_UNSUPPORTED", "DISPATCH_SUBJECT_MISMATCH", "DISPATCH_OBSERVATION_CONFLICT", "DISPATCH_SHADOW_CONFLICT", "DISPATCH_SHADOW_ACTION_CONFLICT", "DISPATCH_SHADOW_RULE_NOT_ACTIVE", "DISPATCH_SHADOW_OBSERVATION_CONFLICT", "DISPATCH_SHADOW_COMPARISON_CONFLICT", "DISPATCH_PROMOTION_RULE_NOT_SHADOW", "DISPATCH_PROMOTION_EVALUATION_UNAVAILABLE"].includes(code) || code.startsWith("OPERATION_UNAVAILABLE:") ? 409 : ["TASK_ARCHIVED", "ARTIFACT_EXPIRED", "OFFICE_DISABLED", "BRAIN_V2_DISABLED"].includes(code) ? ["OFFICE_DISABLED", "BRAIN_V2_DISABLED"].includes(code) ? 503 : 410 : code === "PREVIEW_UNSUPPORTED" ? 415 : code === "REQUEST_TOO_LARGE" || code === "DISPATCH_REQUEST_TOO_LARGE" ? 413 : code === "TARGET_UNKNOWN" || code === "TEST_PARAMETER_UNSUPPORTED" ? 422 : code.startsWith("INVALID_") || code.startsWith("UNKNOWN_") || code === "REGISTRATION_SECRET_TOO_SHORT" || code === "INVALID_JSON_BODY" || code === "MISSING_IDEMPOTENCY_KEY" || code === "INVALID_COMMAND_FAILURE" || code.startsWith("MISSING_OPERATION_PARAMETER") || code === "MISSING_SOURCE_INTENT_KEY" || code === "MISSING_SOURCE_INTENT_REF" || code === "CONTRACT_UPGRADE_REQUIRED" || code === "MCP_METHOD_NOT_FOUND" || code === "REVISION_REQUIRED" || code === "BRAIN_V2_REQUIRED" || code.includes("must be") || code === "WORKSPACE_CONFLICT" || code === "INVALID_COMMAND" || code === "TASK_TYPE_CAPABILITY_MISMATCH" || code === "WORKSPACE_REQUIRED_FOR_CODEX" || code.startsWith("INVALID_DISPATCH_") || ["DISPATCH_OBSERVATION_SOURCE_INVALID", "DISPATCH_OBSERVATION_NOT_SINGLE_ACTION", "DISPATCH_LABEL_SOURCE_INVALID", "DISPATCH_LABEL_EVIDENCE_REQUIRED", "DISPATCH_EVALUATION_THRESHOLD_INVALID", "DISPATCH_EVALUATION_EVIDENCE_INVALID", "DISPATCH_DEGRADATION_WINDOW_INVALID"].includes(code) ? 400 : 500;
