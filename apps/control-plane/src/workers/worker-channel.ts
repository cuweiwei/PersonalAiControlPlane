...
export class WorkerCoordinator {
  ...
  private async handleMessage(connection: SocketRecord, raw: string): Promise<void> {
    ...
    try {
      ...
      else if (type === "task.result") { this.tasks.result(taskId, attemptId, workerId, record(message.result), record(message.metrics), Date.now(), record(message.result_manifest ?? message.resultManifest), message.fence_epoch === undefined ? undefined : Number(message.fence_epoch)); connection.socket.send(JSON.stringify({ type: "task.result.ack", task_id: taskId, attempt_id: attemptId, fence_epoch: message.fence_epoch ?? null })); }
