...
export class WorkerService {
  ...
  updateCapabilities(workerId: string, values: Record<string, any>[], now = Date.now()): void {
    ...
    this.db.transaction(() => {
      ...
      for (const item of incoming) {
        ...
        const descriptorIdentity = { ...item.descriptor }; for (const key of ["status", "evidence_state", "evidenceState", "verification_expires_at", "verificationExpiresAt", "verification_ref", "verificationRef", "contract_version", "contractVersion"]) delete descriptorIdentity[key];
        const descriptorHash = digest(descriptorIdentity);
        const prior = this.db.one<Row>("SELECT grant_status, descriptor_hash, evidence_state, verified_at, verification_expires_at, verification_ref FROM worker_capabilities WHERE worker_id = ? AND capability = ? AND runtime IS ?", workerId, item.capability, item.runtime);
        ...
        const advertisedEvidence = String(value.evidence_state ?? value.evidenceState ?? "ADVERTISED");
        const evidenceState = ["ADVERTISED", "VERIFIED", "STALE", "REQUIRES_REVIEW"].includes(advertisedEvidence) ? advertisedEvidence : "ADVERTISED";
        const effectiveEvidence = descriptorChanged ? "REQUIRES_REVIEW" : evidenceState;
        const verifiedAt = effectiveEvidence === "VERIFIED" ? now : prior?.verified_at ?? null;
        const verificationExpiresAt = effectiveEvidence === "VERIFIED" ? Number(value.verification_expires_at ?? value.verificationExpiresAt ?? now + 30_000) : prior?.verification_expires_at ?? null;
        ...
        if (item.capability === "computer.use" && item.runtime === "cua-driver") {
          ...
          const permissions = record(value.permissions); const permissionSnapshot = { accessibility: boundedString(permissions.accessibility, 32), screenRecording: boundedString(permissions.screen_recording ?? permissions.screenRecording, 32), screenCapture: boundedString(permissions.screen_capture ?? permissions.screenCapture, 32), interactiveSession: boundedString(permissions.interactive_session ?? permissions.interactiveSession, 32), status: boundedString(permissions.status, 32) };
  ...
  private publicWorker(row: Row, now: number): Record<string, unknown> {
    const capabilities = this.db.all<Row>("SELECT id, capability, runtime, runtime_version AS runtimeVersion, max_concurrency AS maxConcurrency, descriptor_json AS descriptor, descriptor_hash AS descriptorHash, grant_status AS grantStatus, superseded_at AS supersededAt, status, contract_version AS contractVersion, evidence_state AS evidenceState, verified_at AS verifiedAt, verification_expires_at AS verificationExpiresAt, verification_ref AS verificationRef FROM worker_capabilities WHERE worker_id = ? ORDER BY capability, runtime", row.id).map((item) => ({ ...item, runtime: item.runtime || null, descriptor: json(item.descriptor), supersededAt: iso(item.supersededAt), verifiedAt: iso(item.verifiedAt), verificationExpiresAt: iso(item.verificationExpiresAt) }));
