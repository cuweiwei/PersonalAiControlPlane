...
export class MissionCoordinator {
  ...
  context(commandId: string): Record<string, unknown> | undefined {
    ...
    return { commandId: row.id, missionId: row.mission_id, missionRunId: row.mission_run_id, authorityEpoch: row.authority_epoch, kind: row.kind, goal: row.goal, title: row.title, objective: parseJson(row.objective_snapshot_json), scope: parseJson(row.scope_snapshot_json), limits: parseJson(row.limits_snapshot_json), activePlanRevision: row.active_plan_revision ?? null, envelope: parseJson(row.envelope_json) };
  }
