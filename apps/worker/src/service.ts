...
import { WorkerWebSocketTransport, WorkerTransportError } from "./transport.ts";
import { WorkerEnrollment } from "./enrollment.ts";
...
export function createWorkerDaemon(options: WorkerServiceOptions): { daemon: WorkerDaemon; db: WorkerLocalDatabase; enrollment: WorkerEnrollment } {
  const db = new WorkerLocalDatabase(join(options.dataDir, "worker.db"));
  const enrollment = new WorkerEnrollment({ dataDir: options.dataDir, origin: options.origin, name: options.name ?? hostname(), platform: platform(), hostname: hostname(), agentVersion: "2.0.0", hardware: { cpu: cpus().map((cpu) => cpu.model), memory_mb: Math.floor(totalmem() / 1_048_576), architecture: arch() } });
