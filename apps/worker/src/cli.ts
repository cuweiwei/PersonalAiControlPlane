...
if (command === "enroll") { const pending = await service.enrollment.ensure(); json(pending ? { status: "approved", workerId: pending.workerId } : { status: "waiting", message: "Registration request created or waiting for owner approval.", next: "Personal AI Control Plane -> Workers -> Approve" }); service.db.close(); }
else if (command === "reset") { service.enrollment.reset(); json({ status: "reset", dataDir: root }); service.db.close(); }
else if (command === "status") { const token = service.enrollment.readToken(); json({ worker: arg("--name") ?? hostname(), platform: platform(), architecture: arch(), enrolled: Boolean(token), workerId: token?.workerId ?? null, origin: origin() }); service.db.close(); }
