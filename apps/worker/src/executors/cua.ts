...
export class CuaDriverExecutor implements WorkerExecutor {
  ...
  async discover(): Promise<{ capabilities: Record<string, JsonValue>[]; models: Record<string, JsonValue>[] }> {
    ...
    try {
      ...
      const permissionsGranted = permissionState.accessibility === "granted" && (permissionState.screen_recording === "granted" || permissionState.screenRecording === "granted");
      ...
      return { capabilities: [{ capability: "computer.use", runtime: "cua-driver", status, evidence_state: permissionsGranted && this.supportedOperations.has("observe") ? "VERIFIED" : "ADVERTISED", permissions: { accessibility: boundedText(permissionState.accessibility ?? "unknown", 32), screen_recording: boundedText(permissionState.screen_recording ?? permissionState.screenRecording ?? "unknown", 32), status: boundedText(permissionState.status ?? "unknown", 32) }, max_concurrency: 1, descriptor: { capability: "computer.use", runtime: "cua-driver", runtime_version: versionText, driver_version: versionText, schema_version: 1, operations: [...this.supportedOperations], desktop_kinds: ["desktop", "window"], capture_scopes: ["window", "desktop"], delivery_modes: ["background", "foreground"], manifest_hash: manifestHash } }], models: [] };
    } catch (error) {
