...
export function createIdentityHttpServer(options: { db: IdentityDatabase; identity: IdentityService; passkeyConfigured: boolean }) {
  return createServer((req: IncomingMessage, response: ServerResponse) => {
    ...
    if (req.method === "GET" && path === "/health/ready") {
      ...
      const ready = schema?.version === 1 && (options.passkeyConfigured || process.env.NODE_ENV !== "production");
      return json(response, ready ? 200 : 503, { status: ready ? "ok" : "not_ready", schemaVersion: schema?.version ?? null, passkey: options.passkeyConfigured ? "configured" : "disabled" });
    }
