...
Implemented locally: goal admission/idempotency, plan DAG validation and activation, SQLite migrations, task transitions, leases/fencing and reconciliation, transactional outbox claim/retry, audit-chain verification, pure hard-stop policy evaluation, health endpoints, WebAuthn RP registration/authentication with secure sessions and recovery codes, identity/session/CSRF/recovery contracts, approvals and bounded grants, signed worker enrollment and capability profiles, scheduling/proactive gates, Archive/artifact/credential contracts, typed adapters, configuration/observability/checkpoints/quota/backup contracts, release artifacts, and policy-checked production Compose.

Still evidence-gated: owner enrollment on the production tailnet, enrolled physical workers, Codex/local-model providers, live ContextHub/Hermes/AIHomePlatform connectors, CUA/wake adapters, and production acceptance evidence. The Identity Gateway serves the owner Passkey page at its configured HTTPS origin (for the current NAS route: `https://gnest.taila77e5f.ts.net:9084/`). Production registration also requires the root-controlled one-time `PAI_BOOTSTRAP_TOKEN`; no API-key fallback or unvalidated production mutation is available.
