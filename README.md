...
Production remains evidence-gated: an owner-selected opaque signing-key authority and workload enrollment, physical-worker OS-vault proof/finalization and capability adapters, Codex/local-model provider evidence, live ContextHub/Hermes connector operations, CUA/wake adapters, immutable release evidence, backup/restore proof, and live acceptance. The three runtime processes join only the AIHomePlatform-owned private edge network; the Identity Gateway serves the owner Passkey page at the exact HTTPS origin configured by `PAI_CANONICAL_ORIGIN`, and successful authentication enters Control Web at `/goals`. Production registration requires the root-controlled one-time `PAI_BOOTSTRAP_TOKEN`. The Orchestrator and Identity Gateway deliberately report not-ready while required external ports are absent. No API-key fallback, embedded production key, fake readiness flag, or unvalidated production mutation is available.
